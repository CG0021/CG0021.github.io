...
function buildFinDataForHospital(hospital) {
    ...
    return allRows.map(row => {
        ...
        return {
            ...
            L_Trade_MedSupply: getVal('A2111020'), L_Trade_Lab: getVal('A2111030'),
            ...
            R_UC_Total: getVal('A410S'), R_UC_OP: getVal('A4101040'), R_UC_PP: getVal('A4101043'),
            R_UC_IP: getVal('A4101080'), R_UC_CR: getVal('A4102050.30'), R_CS: getVal('A4131050.0'),
            R_SS: getVal('A414301S'), R_OtherGeneral: getVal('A9010S'),
            ...
            E_Op_Total: getVal('A519D'), E_Other: getVal('A60SS'),
...
function openFinancialChartModal(hospitalCode) {
    ...
    const rowObjects = allRows.map(row => {
        ...
        return {
            ...
            OPD_Visits_TPS: (function () {
                const tpsRows = Object.values(hospital.data || {});
                const tpsHMap = hospital.headersMap || {};
                const targetTPSHeader = "S_1.3.1.1_จำนวนครั้งผู้ป่วยนอก (ครั้ง)";
            ...
            IPD_Days_TPS: (function () {
                const tpsRows = Object.values(hospital.data || {});
                const tpsHMap = hospital.headersMap || {};
                const targetTPSHeader = "S_1.3.3.1_จำนวนวันนอน";
                const row = tpsRows.find(r => String(r[2]) === String(yearVal) && String(r[3]) === String(quarterVal));
                if (!row) return 0;
                const h = tpsHMap[yearVal] || sourceHeaders;
                const idx = h.indexOf(targetTPSHeader);
            ...
            SumAdjRW: (function () {
                const tpsRows = Object.values(hospital.data || {});
                const tpsHMap = hospital.headersMap || {};
                const targetTPSHeader = "S_1.3.3.2_Sum AdjRW";
                const row = tpsRows.find(r => String(r[2]) === String(yearVal) && String(r[3]) === String(quarterVal));
                if (!row) return 0;
                const h = tpsHMap[yearVal] || sourceHeaders;
                const idx = h.indexOf(targetTPSHeader);
            ...
            BedOccupancy: (function () {
                const tpsRows = Object.values(hospital.data || {});
                const tpsHMap = hospital.headersMap || {};
                const targetTPSHeader = "S_1.3.3.1_อัตราครองเตียง";
                const row = tpsRows.find(r => String(r[2]) === String(yearVal) && String(r[3]) === String(quarterVal));
                if (!row) return 0;
                const h = tpsHMap[yearVal] || sourceHeaders;
                const idx = h.indexOf(targetTPSHeader);
    ...
    window.finCompareHospitals = []; // Reset comparisons on fresh open
...
function updateFinancialChart() {
    ...
    const mkDS = (label, dataMap, color, borderColor, stack, hidden, customKey, subgroupId) => ({
        ...
        yAxisID: (stack === 'Services' || subgroupId === 'OPDStats' || subgroupId === 'IPDStats' || group_id_check === 'Services') ? 
        ...
        sortedSubgroups.forEach(subgroup => {
            ...
            if (mode === 'summary') {
                ...
                datasets.push({
                    ...
                    backgroundColor: window.finChartTypeLine ? s.color + '40' : s.color,
                    _originalColor: window.finChartTypeLine ? s.color + '40' : s.color,
                    borderColor: s.border,
                    _originalBorder: s.border,
                    borderWidth: window.finChartTypeLine ? 2 : 1,
                    fill: false,
                    stack: window.finChartTypeLine ? null : currentStack,
                ...
                items.forEach(item => {
                    ...
                    datasets.push({
                        ...
                        backgroundColor: window.finChartTypeLine ? item.color + '40' : item.color,
                        _originalColor: window.finChartTypeLine ? item.color + '40' : item.color,
        ...
        afterDatasetsDraw(chart, args, options) {
            ...
            chart.data.datasets.forEach((dataset, i) => {
                ...
                meta.data.forEach((element, index) => {
                    ...
                    const centerY = isLine ? y - 12 : (y + base) / 2;
                    ...
                    if (isLine) {
                         ...
                         ctx.shadowBlur = 4;
                         // Match the line color, darker fallback if none
