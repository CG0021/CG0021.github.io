...
window.mgmtNonCumulative = false;
...
const FIXED_HEADERS = [
    ...
    "S_1.3.3.1_จำนวนเตียง", "S_1.3.3.1_CMI", "S_1.3.3.1_อัตราครองเตียง", "S_1.3.3.1_จำนวนวันนอน", "1.3.3.1 อัตราครองเตียงผู้ป่วยใน ≥ 80 %",
...
const MGMT_HEADERS = [
    ...
    "A2111010,เจ้าหนี้การค้าค่ายา", "A2111020,เจ้าหนี้การค้าค่าเวชภัณฑ์มิใช่ยา", "A2111030,เจ้าหนี้การค้าค่าเวชภัณฑ์มิใช่ยา-วัสดุวิทยาศาสตร์การแพทย์", "A2111040,เจ้าหนี้การค้าค่าวัสดุอื่นๆ", //"A2111031,เจ้าหนี้การค้า GR/IR ยาและเวชภัณฑ์มิใช่ยา",
    ...
    "A410S,รวมรายได้ UC",
    "A4101040,รายได้ค่ารักษาพยาบาล UC-OP ใน CUP สุทธิ",
    "A4100001,รายได้จากกองทุน UC-OP เหมาจ่ายต่อผู้มีสิทธิ",
    "A4101010,รายได้ค่ารักษาพยาบาล UC-OP ใน CUP (Gross)",
    "A4101020,หัก ส่วนต่างค่ารักษา UC-OP",
    "A4101021,หัก ส่วนปรับลดค่าแรง OP",

    "A4101043,รายได้ค่ารักษา P&P สุทธิ",
    "A4101040.1,รายได้จากกองทุน UC-P&P เหมาจ่ายต่อผู้มีสิทธิ",
    "A4101041,รายได้ค่ารักษา P&P",
    "A4101042,หัก ส่วนต่างค่ารักษา UC-P&P",
    "A4101042.1,หัก ส่วนปรับลดค่าแรง P&P",

    "A4101080,รายได้ค่ารักษาพยาบาล UC-IP เหมาจ่ายรายหัว สุทธิ",
    "A4101050,รายได้ค่ารักษาพยาบาล UC-IP เหมาจ่ายรายหัว (Gross)",
    "A4101060,หัก ส่วนต่างค่ารักษา UC-IP (DRG)",
    "A4101061,หัก ส่วนปรับลดค่าแรง IP",
    "A4101070,บวก ส่วนต่างค่ารักษา UC-IP",

    "A4102040,รายได้จากการเรียกเก็บ UC สุทธิ",
    "A4102010,รายได้จากการเรียกเก็บ UC (Gross)",
    "A4102020,หัก ส่วนต่างค่ารักษา OP UC",
    "A4102030,บวก ส่วนต่างค่ารักษา OP UC",

    "A4102050.30,รายได้ค่ารักษา UC - บริการเฉพาะ (CR)- สุทธิ",
    "A4102041,รายได้ UC-CR (Gross)",
    "A4102050,หัก ส่วนต่าง UC-CR",
    "A4102050.10,หัก ส่วนปรับลดค่าแรง CR",
    "A4102050.20,บวก ส่วนต่าง UC-CR",
    ...
    "A4103101,รายได้จากการยกหนี้กรณีส่งต่อผู้ป่วยระหว่าง รพ.",
    ...
    "A519N,รวมค่าใช้จ่ายในการดำเนินงาน (ไม่รวมค่าเสื่อมราคาและค่าตัดจำหน่าย)",
...
const MGMT_GROUPS = {
    ...
    //"A2111_M": ["A2111010", "A2111020", "A2111030", "A2111031", "A2111040"],
    ...
    //รายได้
    "A49": ["A419S", "A4201010", "A9010S"],

    //รวมรายได้ค่ารักษาพยาบาล
    "A419S": ["A410S", "A4131050.0", "A413105S", "A414301S", "A4153S", "A4161S", "A4171010", "A4191010", "A4191011", "A4192010"],
    ...
    "A4131050.0": ["A4121010", "A4131010", "A4131050"],
    ...
    "A5009D": ["A5009N", "A5002010", "A5002020", "A5002030"],
    ...
    "A519D": ["A519N", "A5102010", "A5102020", "A5102030"],
    ...
    "EBITDA": ["A911S", "A912S", "A91N", "E400S", "E500S", "A91D", "A90S"],
...
function populateYearFilters() {
    ...
    startYearSelect.value = 2565; // Default start year as requested
...
let hospitalQueue = [];
let serviceQueue = [];
let activeHospitalLoads = 0;
let activeServiceLoads = 0;
const MAX_CONCURRENT_HOSPITALS = 10;

function selectHospital(code, silent = false) {
    document.getElementById('suggestions').style.display = 'none';
    if (document.getElementById('searchInput')) document.getElementById('searchInput').value = '';

    // Find or add the hospital to the comparison list immediately so the column appears
    ...
    // Set loading states
    if (hospitalItem) {
        hospitalItem.isLoading = true;         // Primary API fetch
        hospitalItem.isLoadingHistory = true;  // Historical API fetch
        hospitalItem.isLoadingService = true;  // Service data fetch
        hospitalItem.queueState = 'waiting_financial'; // Clearly track state for UI
    }

    if (!silent) renderMainContent(); // Show the new column with spinners immediately

    hospitalQueue.push({ code, silent, hospitalItem });
    processQueues();
}

function processQueues() {
    // 1. Process hospital queue (strictly 1 by 1)
    while (activeHospitalLoads < MAX_CONCURRENT_HOSPITALS && hospitalQueue.length > 0) {
        const task = hospitalQueue.shift();
        activeHospitalLoads++;

        task.hospitalItem.queueState = 'loading_financial';
        if (!task.silent) renderMainContent('4');

        _doSelectHospital(task.code, task.silent, task.hospitalItem)
            .then(() => {
                // After financial data is loaded completely for THIS hospital
                task.hospitalItem.queueState = 'waiting_service';
                serviceQueue.push(task.hospitalItem);
            })
            .catch(e => {
                console.error("Error loading hospital:", e);
                alert(`เกิดข้อผิดพลาดในการโหลดข้อมูลโรงพยาบาล: ${e.message}`);
                removeHospital(task.code);
            })
            .finally(() => {
                activeHospitalLoads--;
                processQueues(); // Trigger next in queue
            });
    }

    // 2. Process service queue ONLY if hospital queue is empty AND no active hospital loads
    if (activeHospitalLoads === 0 && hospitalQueue.length === 0) {
        while (activeServiceLoads < 1 && serviceQueue.length > 0) {
            const item = serviceQueue.shift();
            activeServiceLoads++;

            item.queueState = 'loading_service';
            renderMainContent('4');

            // Background load Service Statistics (OPD/IPD) data
            loadServiceDataInBackground(item)
                .catch(e => {
                    console.warn(`Background service data loading failed for ${item.code}:`, e);
                })
                .finally(() => {
                    item.isLoadingService = false;
                    item.queueState = 'done';
                    renderMainContent('4');
                    activeServiceLoads--;
                    // Small delay before next service to let UI breathe
...
async function _doSelectHospital(code, silent, hospitalItem) {
    try {
        // We will fetch ALL years as a single lump sum to make it faster
        const fastPromises = [
            jsonpRequest('getMgmtData', `code=${code}`).catch(e => ({ headers: [], data: {} })),
            jsonpRequest('getAllYearlyData', `code=${code}`).catch(e => ({ headers: [], data: {} }))
        ];

        const [fastMgmt, fastTps] = await Promise.all(fastPromises);

        let info = hospitalList.find(h => String(h.c) === String(code));
        if (info) hospitalItem.hospitalInfo = info;

        if (!hospitalItem.data) hospitalItem.data = {};
        if (!hospitalItem.mgmtData) hospitalItem.mgmtData = {};
        if (!hospitalItem.headersMap) hospitalItem.headersMap = {};
        if (!hospitalItem.mgmtHeadersMap) hospitalItem.mgmtHeadersMap = {};

        // Apply ALL TPS Data directly
        if (fastTps.data) {
            hospitalItem.data = { ...hospitalItem.data, ...fastTps.data };
            if (fastTps.headers && fastTps.headers.length > 0) {
                // Populate headers map for any year found in the data
                const yIdx = fastTps.headers.findIndex(h => String(h).trim() === 'ปี');
                if (yIdx >= 0) {
                    for (const row of Object.values(fastTps.data)) {
                        const y = String(row[yIdx]);
                        if (y) hospitalItem.headersMap[y] = fastTps.headers;
                    }
                }
                if (comparisonList.length <= 1 && sourceHeaders.length === 0) {
                    sourceHeaders = fastTps.headers;
                }
            }
        }

        // Apply ALL Mgmt Data directly
        if (fastMgmt.data) {
            hospitalItem.mgmtData = { ...hospitalItem.mgmtData, ...fastMgmt.data };
            if (fastMgmt.headers && fastMgmt.headers.length > 0) {
                const yIdx = fastMgmt.headers.findIndex(h => String(h).trim() === 'ปี');
                if (yIdx >= 0) {
                    for (const row of Object.values(fastMgmt.data)) {
                        const y = String(row[yIdx]);
                        if (y) hospitalItem.mgmtHeadersMap[y] = fastMgmt.headers;
                    }
                }
            }
        }

        hospitalItem.isLoading = false;
        hospitalItem.isLoadingHistory = false; // All history is loaded in one go

        const allRows = Object.values(hospitalItem.data || {});
        // Find default headers just in case we need name info
        let currentHeaders = fastTps.headers || sourceHeaders || [];
        // If we don't have current headers from this TPS fetch, try to find it in the headers map
        if (currentHeaders.length === 0 && Object.values(hospitalItem.headersMap).length > 0) {
            currentHeaders = Object.values(hospitalItem.headersMap)[0];
        }

        const nameIdx = currentHeaders ? currentHeaders.indexOf('หน่วยบริการ') : -1;

        if (allRows[0] && nameIdx > -1 && allRows[0][nameIdx]) {
            hospitalItem.name = formatHospitalName(allRows[0][nameIdx]);
        } else if (info) {
            hospitalItem.name = formatHospitalName(info.n);
        }

        if (!silent) renderMainContent('4');
...
function getCell(row, header, h) {
    ...
    if (idx === -1) {
        ...
        if (header.startsWith('A2111_M')) {
            ...
            //const codes = ["A2111010", "A2111020", "A2111030", "A2111031", "A2111040"];
...
function getLabelStyle(item) {
    ...
    if (label.includes('S_1.3.1.1_จำนวนครั้งผู้ป่วยนอก (ครั้ง)')) label = 'จำนวนรับบริการ OPD (รายไตรมาส)';
    if (label.includes('S_1.3.1.2_Sum AdjRW')) label = 'Sum AdjRW (รายไตรมาส)';
    if (label.includes('S_1.3.3.2_Sum AdjRW')) label = 'Sum AdjRW (รายไตรมาส)';
    ...
    if (label.includes('1.2.4 การบริหารสินค้าคงคลัง')) label = '1.2.4 การบริหารสินค้าคงคลัง (Inventory Management) ≤ 60 วัน ยกเว้น รพ.พื้นที่เกาะ ≤ 90 วัน';
...
function addToComparison(code, data, isLoading = false, hMap = null) {
    ...
    // Apply formatting to clean the title
    if (name !== 'กำลังโหลด...') name = formatHospitalName(name);
    ...
    mainContent.innerHTML = `${headerHTML}
                ...
                <div class="compare-title-row">
                    <div class="compare-title-group">
                        ...
                        <!-- Non-Cumulative Toggle -->
                        <div class="compare-toggle-wrapper" onclick="document.getElementById('mgmtNonCumulativeToggle').click()">
                            <label class="toggle-switch" style="transform:scale(0.8); transform-origin:left center;" onclick="event.stopPropagation()">
                                <input type="checkbox" id="mgmtNonCumulativeToggle" ${window.mgmtNonCumulative ? 'checked' : ''} onchange="window.mgmtNonCumulative = this.checked; toggleHeatmapAndPreserveState()">
                                <span class="slider"></span>
                            </label>
                            <span class="compare-toggle-label">แสดงค่ารายไตรมาส (ไม่สะสม)</span>
                        </div>
                    </div>
                    <!-- Open Chart Button -->
...
function toggleHeatmapAndPreserveState() {
    ...
    // Save current non-cumulative toggle state if present
    const nonCumToggle = document.getElementById('mgmtNonCumulativeToggle');
    if (nonCumToggle) {
        window.mgmtNonCumulative = nonCumToggle.checked;
    }
    renderMgmtTable();
...
function renderMgmtTable() {
    ...
    cols.forEach(c => {
        ...
        let spinnerTitle = 'กำลังโหลดข้อมูล...';
        if (c.queueState === 'waiting_financial') spinnerTitle = 'กำลังรอคิวโหลดข้อมูล...';
        if (c.queueState === 'loading_financial') spinnerTitle = 'กำลังโหลดข้อมูลทุกปี...';
        if (c.queueState === 'waiting_service') spinnerTitle = 'รอคิวโหลดสถิติบริการ...';
        if (c.queueState === 'loading_service') spinnerTitle = 'กำลังโหลดสถิติบริการ...';

        const isStillLoading = c.isLoading || c.isLoadingHistory || c.isLoadingService;
        const spinner = isStillLoading && !c.isLoading ? `<span class="spinner-sm" style="display:inline-block; width:12px; height:12px; border-width:2px; vertical-align:middle; margin-left:5px;" title="${spinnerTitle}"></span>` : '';
        html += `<th class="th-min-width">${c.isLoading ? '<span class="spinner-sm"></span> ' + c.name : '<div class="header-cell-wrapper">' + c.name + spinner + chartBtn + removeBtn + '</div>'}</th>`;
    });
    html += `</tr></thead><tbody>`;
    ...
    MGMT_HEADERS.forEach((rawHeader, idx) => {
        ...
        let rowStyles = [];
        rowStyles.push(`padding-left: ${depth * 1.5 + 1}rem;`);

        let trClasses = [`depth-${Math.min(depth, 4)}`, isParent ? 'parent-row' : 'child-row'];

        let sectionClass = '';
        if (code === 'A1291' || code.startsWith('A1')) sectionClass = 'section-assets';
        else if (code === 'A390' || code === 'A291' || code.startsWith('A2')) sectionClass = 'section-liabilities';
        else if (code === 'A32S' || code.startsWith('A3')) sectionClass = 'section-equity';
        else if (code === 'A49' || code.startsWith('A4') || code.startsWith('A7') || code.startsWith('A8') || code.startsWith('A90')) sectionClass = 'section-income';
        else if (code === 'A5009D' || code.startsWith('A50')) sectionClass = 'section-expenses'; // Costs -> Amber
        else if (code === 'A519D' || code.startsWith('A51') || code.startsWith('A6')) sectionClass = 'section-costs';    // Expenses -> Red
        else if (code === 'EBITDA' || code.startsWith('E4') || code.startsWith('E5') || code.startsWith('A91')) sectionClass = 'section-ebitda';

        if (sectionClass) trClasses.push(sectionClass);

        let trStyles = [];
        let tdStyle = `padding-left: ${depth * 1.5 + 1}rem;`;

        if (isParent) {
            trStyles.push('cursor:pointer');
        ...
        if (parentCode) {
            trClasses.push('display-none');
            trStyles.push('display:none');
        } else {
            trStyles.push('display:table-row');
        }

        let trAttrs = `class="${trClasses.join(' ')}" data-account-code="${code}"`;
        if (isParent) {
            trAttrs += ` onclick="toggleMgmtRow('${code}', this)"`;
        }
        if (parentCode) {
            trAttrs += ` data-mgmt-parent="${parentCode}"`;
        }
        if (trStyles.length > 0) {
            trAttrs += ` style="${trStyles.join('; ')}"`;
        }

        // Re-defining style variable for compatibility with the html += line
        const style = tdStyle;
        ...
        const rowData = cols.map(hospital => {
            ...
            let val = getCell(targetRow, rawHeader, h);

            // --- NON-CUMULATIVE CALCULATION ---
            if (window.mgmtNonCumulative && quarter !== '1' && quarter !== 'any') {
                const isFlowItem = ['section-income', 'section-expenses', 'section-costs', 'section-ebitda'].includes(sectionClass);
                if (isFlowItem) {
                    const qInt = parseInt(quarter);
                    if (!isNaN(qInt) && qInt > 1) {
                        let prevRow = null;
                        let yIdx = h ? h.findIndex(sh => String(sh).trim() === 'ปี') : -1;
                        if (yIdx === -1) yIdx = 2;
                        let qIdx = h ? h.findIndex(sh => String(sh).trim() === 'ไตรมาส') : -1;
                        ...
                        for (let r of allRows) {
                            const rowYear = String(r[yIdx] || "").trim();
                            const rowQuarter = String(r[qIdx] || "").trim();
                            if (rowYear === String(comparisonYear) && rowQuarter === String(qInt - 1)) {
                                prevRow = r;
                                break;
                            }
                        }
                        ...
                        if (prevRow) {
                            const prevVal = getCell(prevRow, rawHeader, h);
                            const curNum = parseFloat(String(val).replace(/,/g, ''));
        ...
        rowData.forEach((d, idx) => {
            if (d.isLoading) {
                html += `<td class="text-center" style="vertical-align:middle;"><span class="spinner-sm" style="display:inline-block; width:14px; height:14px; border-width:2px; border-top-color:var(--primary-color);"></span></td>`;
                return;
            }
    ...
    if (isCurrentlyExpanded) {
        ...
        descendants.forEach(descendant => {
            ...
            if (!descendant.classList.contains('row-pinned')) {
                ...
                descendant.classList.add('display-none');
            }
...
function renderTableBodyComparison() {
    ...
    cols.forEach(c => {
        ...
        const isStillLoading = c.isLoading || c.isLoadingHistory || c.isLoadingService;
        const spinner = isStillLoading && !c.isLoading ? `<span class="spinner-sm" style="display:inline-block; width:12px; height:12px; border-width:2px; vertical-align:middle; margin-left:5px;" title="${spinnerTitle}"></span>` : '';
        html += `<th>${c.isLoading ? '<span class="spinner-sm"></span> ' + c.name : c.name + spinner + ' ' + removeBtn}</th>`;
    });
    html += `</tr></thead><tbody>`;
    ...
    items.forEach(item => {
        ...
        cols.forEach(hospital => {
            if (hospital.isLoading) {
                html += `<td class="loading-cell text-center" style="vertical-align:middle;"><span class="spinner-sm" style="display:inline-block; width:14px; height:14px; border-width:2px; border-top-color:var(--primary-color);"></span></td>`;
                return;
            }
            let targetRow = null;
...
function openChartModal(input, labelInput = null, unused = null, sourceType = 'mgmt') {
    ...
    if (existing) {
        ...
        if (document.getElementById('showChartNonCumulative')) {
            document.getElementById('showChartNonCumulative').checked = !!opts.showNonCumulative;
        }
        ...
        const modalHTML = `
                    <div id="chartModal" class="modal" style="display:block;">
                                ...
                                <div style="flex: 1; display: flex; align-items: center; gap: 1rem;">
                                    ...
                                    <div style="display:flex; gap: 0.5rem; margin-left: auto; margin-right: 2rem; align-items: center;">
                                        ...
                                        <div style="display:flex; gap:10px; margin-right: 1rem; border-right: 1px solid var(--border-color); padding-right: 1rem;">
                                            ...
                                            <!-- Non-Cumulative Toggle -->
                                            <div style="display:flex; align-items:center; gap:8px; cursor:pointer;" onclick="document.getElementById('showChartNonCumulative').click()">
                                                <label class="toggle-switch" onclick="event.stopPropagation()">
                                                    <input type="checkbox" id="showChartNonCumulative" ${window.currentChartData.options.showNonCumulative ? 'checked' : ''} onchange="toggleChartOption('showNonCumulative')">
                                                    <span class="slider"></span>
                                                </label>
                                                <span style="font-weight:500; font-size:0.9rem; color:var(--text-main);">รายไตรมาส (ไม่สะสม)</span>
                                            </div>
                                        </div>
...
if (!window.chartOptions) {
    window.chartOptions = { showValues: true, showDiff: true, useAbbreviated: true, showNonCumulative: false };
}

function toggleChartOption(option) {
    const checkbox = document.getElementById(option === 'showValues' ? 'showChartValues' : (option === 'showDiff' ? 'showChartDiff' : (option === 'showNonCumulative' ? 'showChartNonCumulative' : 'showChartAbbreviated')));
...
function renderComparisonChart(animationMode = 'progressive') {
    ...
    setTimeout(() => {
        ...
        if (ctx && window.Chart) {
            ...
            // Flag: labels are only drawn after animation finishes
            window._comparisonLabelsReady = (animationMode !== 'progressive');
            clearTimeout(window._comparisonLabelTimer);

            if (animationMode === 'progressive') {
                const delayPerPoint = 20;
                const pointDuration = 100;
                // Total time for all points to finish + small buffer
                const totalAnimMs = (filteredLabels.length > 0 ? filteredLabels.length - 1 : 0) * delayPerPoint + pointDuration + 80;
                ...
                // Show labels once all animation has finished
                window._comparisonLabelTimer = setTimeout(() => {
                    window._comparisonLabelsReady = true;
                    if (window.comparisonChartInstance) window.comparisonChartInstance.draw();
                }, totalAnimMs);
            } else {
                // No Animation (Instant) — labels ready immediately
            ...
            // Custom Plugin for Drawing Text (line chart in comparison view)
            const dataLabelPlugin = {
                id: 'customDataLabel',
                afterDatasetsDraw(chart, args, options) {
                    // Wait until animation finishes before drawing labels
                    if (!window._comparisonLabelsReady) return;
                    ...
                    chart.data.datasets.forEach((dataset, i) => {
                        ...
                        meta.data.forEach((element, index) => {
                            ...
                            // Exact x position from element (no stagger — stagger causes misalignment)
                            const props = element.getProps(['x'], true);
                            const x = props.x;

                            // Skip elements outside visible chart area (zoom/pan)
                            if (x < chartArea.left || x > chartArea.right) return;

                            // Exact Y from scale (line chart — no stacking)
                            const yPos = scale.getPixelForValue(val);

                            ctx.save();
                            ctx.textAlign = 'center';

                            // --- Value label: drawn just above the data point ---
                            if (showValues) {
                                ...
                                ctx.shadowBlur = 4;
                                // Clamp vertically so label doesn't clip out of chartArea
                                const labelY = Math.max(chartArea.top + 12, yPos - 5);
                                ctx.fillText(formatAbbreviatedDirect(val, useAbbreviated), x, labelY);
                            }

                            // --- Diff label: between current and previous point ---
                            if (showDiff && index > 0) {
                                const prevVal = dataset.data[index - 1];
                                if (prevVal !== null && prevVal !== undefined) {
                                    const diff = val - prevVal;
                                    const prevProps = meta.data[index - 1].getProps(['x'], true);
                                    const midX = (x + prevProps.x) / 2;
                                    ...
                                    const prevYPos = scale.getPixelForValue(prevVal);
                                    const diffY = Math.max(chartArea.top + 10,
                                        showValues ? yPos - 18 : yPos - 5);
                                    ...
                                    ctx.shadowColor = 'rgba(255,255,255,0.9)';
                                    ctx.shadowBlur = 3;
                                    ctx.fillText(diffStr, midX, diffY);
                                }
                            }
                            ctx.restore();
                        });
            ...
            const datasets = [];
            const useNonCumulative = window.currentChartData.options.showNonCumulative;
            ...
            selectedHospitals.forEach((hosp, hIdx) => {
                ...
                indicators.forEach((ind, iIdx) => {
                    ...
                    const dataPoints = filteredLabels.map(k => {
                        ...
                        let val = d ? d.num : null;

                        if (useNonCumulative && val !== null) {
                            ...
                            if (parts.length === 2) {
                                ...
                                if (q > 1) {
                                    const prevK = `${year}-Q${q - 1}`;
                                    const prevD = allChartData[hosp.code][ind.code][prevK];
                                    if (prevD && prevD.num !== null) {
                                        val = val - prevD.num;
                                    }
                                }
                            }
            ...
            if (window.comparisonChartInstance) {
                ...
                // Create Negative Zone Plugin
                const negativeZonePlugin = {
                    id: 'negativeZone',
                    beforeDraw(chart) {
                        const isEbitda = window.currentChartData && window.currentChartData.indicators && window.currentChartData.indicators.some(ind => ind.label.includes('EBITDA') || ind.label.includes('หัก ค่าใช้จ่าย (ไม่รวมค่าเสื่อมราคาและค่าตัดจำหน่าย)'));
                        if (!isEbitda) return;
                        ...
                        // If 0 is within or above the chart area, draw red below it
                        if (yZero < chartArea.bottom) {
                            ctx.save();
                            ctx.fillStyle = 'rgba(239, 68, 68, 0.08)'; // Light red background
                            // The fill area should be from yZero down to the bottom of the chart area
                            const fillTop = Math.max(yZero, chartArea.top);
                            ctx.fillRect(chartArea.left, fillTop, chartArea.right - chartArea.left, chartArea.bottom - fillTop);
                ...
                window.comparisonChartInstance = new Chart(ctx, {
                    ...
                    plugins: [yearBackgroundPlugin, dataLabelPlugin, negativeZonePlugin],
                    options: {
                        responsive: true,
...
function buildFinDataForHospital(hospital) {
    if (!hospital) return [];
    ...
    return allRows.map(row => {
        ...
        const h = headersMap[yearVal] || firstHeaders;
        ...
        const getTPS = (headerLabel) => {
            const tpsRows = Object.values(hospital.data || {});
            const tpsHMap = hospital.headersMap || {};
            const r = tpsRows.find(rw => String(rw[2]) === String(yearVal) && String(rw[3]) === String(quarterVal));
            if (!r) return 0;
            const ht = tpsHMap[yearVal] || sourceHeaders;
            const idx = ht.indexOf(headerLabel);
            if (idx === -1) return 0;
            const v = r[idx];
        ...
        return {
            ...
            R_InterAgency_Support: getVal('A7001010'),
            R_Budget_Capital: getVal('A7001020'),
            R_UC_Capital: getVal('A7001030'),
            R_Budget_Others: getVal('A7001040'),
            R_Donation: getVal('A7001050'),
            R_Other_Fees: getVal('A7001120'),
            R_Budget_Subsidy: getVal('A8001010'),
            R_InterUnit: getVal('A9001010'),

            R_UC_Total: getVal('A410S'),
            R_UC_OP_Net: getVal('A4101040'),
            R_UC_OP_Gross: getVal('A4101010'),
            R_UC_OP_Diff: getVal('A4101020'),
            R_UC_OP_Salary: getVal('A4101021'),
            R_UC_PP_Net: getVal('A4101043'),
            R_UC_PP_Gross: getVal('A4101041'),
            R_UC_PP_Diff: getVal('A4101042'),
            R_UC_PP_Salary: getVal('A4101042.1'),
            R_UC_IP_Net: getVal('A4101080'),
            R_UC_IP_Gross: getVal('A4101050'),
            R_UC_IP_Diff: getVal('A4101060'),
            R_UC_IP_Salary: getVal('A4101061'),
            R_UC_IP_Add: getVal('A4101070'),
            R_UC_Refer_Net: getVal('A4102040'),
            R_UC_Refer_Gross: getVal('A4102010'),
            R_UC_Refer_Diff: getVal('A4102020'),
            R_UC_Refer_Add: getVal('A4102030'),
            R_UC_CR_Net: getVal('A4102050.30'),
            R_UC_CR_Gross: getVal('A4102050'),
            R_UC_CR_Diff: getVal('A4102050.10'),
            R_UC_CR_Add: getVal('A4102050.20'),
            R_UC_Refer_OP: getVal('A4102010'),
            R_UC_Area: getVal('A4101011'),
            R_UC_PP_Other: getVal('A4101044'),
            R_UC_Specific: getVal('A4101100'),
            R_UC_Performance: getVal('A4101110'),
            R_UC_Other_Misc: getVal('A4101120'),
            R_UC_CF: getVal('A4102051'),
            R_UC_BadDebt_Writeoff: getVal('A4101130'),

            R_CS: getVal('A4131050.0'),
            R_CS_OP: getVal('A4131010'),
            R_CS_IP: getVal('A4131020'),
            R_CS_Diff: getVal('A4131030'),
            R_CS_Add: getVal('A4131040'),
            R_CS_IP_Net: getVal('A4131050'),

            R_LAO_Total: getVal('A413105S'),
            R_LAO_OP: getVal('A4131050.1'),
            R_LAO_IP: getVal('A4131050.2'),
            R_LAO_Diff: getVal('A4131050.3'),
            R_LAO_Add: getVal('A4131050.4'),
            R_LAO_IP_Net: getVal('A4131050.5'),

            R_SS: getVal('A414301S'),
            R_SS_Network_Net: getVal('A4141040'),
            R_SS_Network_Gross: getVal('A4141010'),
            R_SS_Network_Diff: getVal('A4141020'),
            R_SS_NonNetwork: getVal('A4142010'),
            R_SS_Fund: getVal('A4143010'),
            R_SS_Other_Misc: getVal('A4143010.1'),

            R_Alien_Total: getVal('A4153S'),
            R_Alien_Net: getVal('A4151040'),
            R_Alien_Gross: getVal('A4151010'),
            R_Alien_Diff: getVal('A4151020'),
            R_Alien_NonCUP: getVal('A4152010'),
            R_Alien_Fund: getVal('A4153010'),
            R_Alien_Checkup: getVal('A4153011'),

            R_Status_Total: getVal('A4161S'),
            R_Status_NonCUP_Net: getVal('A4161040'),
            R_Status_NonCUP_Gross: getVal('A4161010'),
            R_Status_NonCUP_Diff: getVal('A4161020'),
            R_Status_InCUP: getVal('A4161050'),
            R_Status_Subsidy: getVal('A4161060'),

            R_Med_Act: getVal('A4171010'),
            R_Med_EMS: getVal('A4191011'),
            R_Med_Other_Service: getVal('A4192010'),
            R_Med_Other_Misc: getVal('A4191010'),

            E_Med_Labour: getVal('A5001040') + getVal('A5001050') + getVal('A5001050.1') + getVal('A5001050.2') + getVal('A5001060'),
            E_Med_Material: getVal('A5001010') + getVal('A5001020') + getVal('A5001030') + getVal('A5001031'),
            E_Med_Depre: getVal('A5002010') + getVal('A5002020') + getVal('A5002030'),
            E_Med_Total: getVal('A5009D'),

            // Detailed Medical Costs
            E_Med_Refer_UC: getVal('A5001110'),
            E_Med_Refer_SP: getVal('A5001120'),
            E_Med_Refer_OtherSP: getVal('A5001130'),
            E_Med_Refer_Alien: getVal('A5001140'),
            E_Med_Refer_Status: getVal('A5001150'),
            E_Med_Lab_Service: getVal('A5001160'),

            E_Op_Labour: getVal('A5101010') + getVal('A5101020') + getVal('A5101020.1') + getVal('A5101020.2') + getVal('A5101030'),
            E_Op_Material: getVal('A510104030') + getVal('A510105030') + getVal('A510106030') + getVal('A510107030'),
            E_Op_Depre: getVal('A5102010') + getVal('A5102020') + getVal('A5102030'),
            E_Op_InterUnit: getVal('A5101260'),
            E_Op_InterUnit_Other: getVal('A5101261'),
            E_Op_Other_General: getVal('A5101270'),
            E_Op_Total: getVal('A519D'),

            // Bad Debt / Provisions
            E_Op_BadDebt_Provision: getVal('A5101090') + getVal('A5101100') + getVal('A5101120') + getVal('A5101130') + getVal('A5101140'),
            E_Op_BadDebt_Actual: getVal('A5101170') + getVal('A5101190') + getVal('A5101201') + getVal('A5101210') + getVal('A5101240') + getVal('A5101250'),

            E_Other: getVal('A60SS'),
            E_Other_Project: getVal('A6001020'),
            E_Other_Project_PP: getVal('A6001030'),
            E_Other_Refund: getVal('A6001130'),
            E_Other_Maintenance: getVal('A6001140'),

            OPD_Visits_TPS: getTPS('S_1.3.1.1_จำนวนครั้งผู้ป่วยนอก (ครั้ง)'),
            IPD_Days_TPS: getTPS('S_1.3.3.1_จำนวนวันนอน'),
            SumAdjRW: getTPS('S_1.3.3.2_Sum AdjRW'),
            CMI: getTPS('S_1.3.3.1_CMI'),
            BedOccupancy: getTPS('S_1.3.3.1_อัตราครองเตียง'),
            SumAdjRW_Per_Day: 0
        };
    });
}

window.finCompareHospitals = [];
...
async function loadServiceData(hospital) {
    ...
    years = [...new Set(years)].filter(y => !isNaN(y)).sort();
    ...
    for (let i = 0; i < years.length; i++) {
        ...
        try {
            ...
            if (iData) {
                const getR = (m) => parseInt(String(iD['result' + m] || '0').replace(/,/g, '')) || 0;
                const getT = (m) => parseInt(String(iD['target' + m] || '0').replace(/,/g, '')) || 0;
                iData._totalResult = getR('10') + getR('11') + getR('12') + getR('01') + getR('02') + getR('03') + getR('04') + getR('05') + getR('06') + getR('07') + getR('08') + getR('09');
                iData._totalTarget = getT('10') + getT('11') + getT('12') + getT('01') + getT('02') + getT('03') + getT('04') + getT('05') + getT('06') + getT('07') + getT('08') + getT('09');
...
// ==========================================
// HELPER: Build row objects for any hospital
// ==========================================
function buildFinDataForHospital(hospital) {
    const allRows = Object.values(hospital.mgmtData || {});
    if (allRows.length === 0) return [];
    ...
    return allRows.map(row => {
        ...
        const yearVal = row[yIdx];
        const quarterVal = row[qIdx];
        let h = headersMap[yearVal] || firstHeaders;
        ...
        const getTPS = (header) => {
            const tpsRows = Object.values(hospital.data || {});
            const tpsHMap = hospital.headersMap || {};
            const r = tpsRows.find(r => String(r[2]) === String(yearVal) && String(r[3]) === String(quarterVal));
            if (!r) return 0;
            const th = tpsHMap[yearVal] || [];
            const idx = th.indexOf(header);
            if (idx === -1) return 0;
            const v = r[idx];
            return (v && v !== '-' && v !== '') ? parseFloat(String(v).replace(/,/g, '')) : 0;
        };
        ...
        return {
            year: yearVal, quarter: quarterVal,
            Assets: getVal('A1291'), CurrentAssets: getVal('A119'), Cash: getVal('A1111S'),
            UC_AR: getVal('A11211S'), Inventory: getVal('A1131S'), NonCurrentAssets: getVal('A129'),
            LandBuild: getVal('A1211010'), Equipment: getVal('A1211020'), FixedDeposit: getVal('A1211001'),
            Liabilities: getVal('A291'), L_Trade: getVal('A2111S'), L_Trade_Pharma: getVal('A2111010'),
            L_Trade_MedSupply: getVal('A2111020'), L_Trade_Lab: getVal('A2111030'), // L_Trade_Lab_GRIR: getVal('A2111031'),
            L_Trade_OtherMat: getVal('A2111040'),
            L_Trade_Others: getVal('A2111050') + getVal('A2111060') + getVal('A2111070') + getVal('A2111080') + getVal('A2111090'),
            L_Medical: getVal('A2112S'), L_FundDeposit: getVal('A2122020') + getVal('A2122021') + getVal('A2122022') + getVal('A2122023') + getVal('A2122024'),
            L_AccruedExp: getVal('A2131010'), L_CurrentOtherAll: getVal('A218'), L_NonCurrent: getVal('A29'),
            R_GrandTotal: getVal('A49'), R_Medical: getVal('A419S'), R_Personnel: getVal('A4201010'),
            ...
            // SS Detail breakdown
            R_SS: getVal('A414301S'),
            R_SS_Main: getVal('A4141010'),
            R_SS_Diff: getVal('A4141020'),
            R_SS_Add: getVal('A4141030'),
            R_SS_Net: getVal('A4141040'),
            R_SS_Other: getVal('A4142010'),
            R_SS_Fund: getVal('A4143010'),
            R_SS_Dev: getVal('A4143010.1'),

            // Alien Detail breakdown
            R_Alien_Total: getVal('A4153S'),
            R_Alien_Main: getVal('A4151010'),
            R_Alien_Diff: getVal('A4151020'),
            R_Alien_Add: getVal('A4151030'),
            R_Alien_Net: getVal('A4151040'),
            R_Alien_Other: getVal('A4152010'),
            R_Alien_Fund: getVal('A4153010'),
            R_Alien_Checkup: getVal('A4153011'),

            // Status Issue Detail breakdown
            R_Status_Total: getVal('A4161S'),
            R_Status_Other: getVal('A4161010'),
            R_Status_Diff: getVal('A4161020'),
            R_Status_Add: getVal('A4161030'),
            R_Status_Net: getVal('A4161040'),
            R_Status_InCUP: getVal('A4161050'),
            R_Status_Subsidy: getVal('A4161060'),

            // Other Medical
            R_Other_PRB: getVal('A4171010'),
            R_Other_Medical: getVal('A4191010'),
            R_Other_EMS: getVal('A4191011'),
            R_Other_Service: getVal('A4192010'),
            R_Medical: getVal('A419S'),
            ...
            E_Other_Refund: getVal('A6001130'),
            E_Other_Maintenance: getVal('A6001140'),
            OPD_Result: 0, OPD_Target: 0, IPD_Result: 0, IPD_Target: 0,
            OPD_Visits_TPS: getTPS('S_1.3.1.1_จำนวนครั้งผู้ป่วยนอก (ครั้ง)'),
            IPD_Days_TPS: getTPS('S_1.3.3.1_จำนวนวันนอน'),
            SumAdjRW: getTPS('S_1.3.3.2_Sum AdjRW'),
            CMI: getTPS('S_1.3.3.1_CMI'),
            BedOccupancy: getTPS('S_1.3.3.1_อัตราครองเตียง'),
            SumAdjRW_Per_Day: 0 // Will be calculated after differencing
        };
    });
}
...
// Switch primary hospital without closing modal
window.updateFinHospital = function (newCode) {
    if (!newCode || newCode === window.currentFinHospitalCode) return;
    // Remove new primary from compare list if present
    window.finCompareHospitals = window.finCompareHospitals.filter(c => c !== newCode);
    openFinancialChartModal(newCode);
};

// Toggle compare hospital
window.toggleFinCompare = function (code) {
    // If it is currently being previewed (hovered), lock it in instead of removing it
    if (window.__previewCompareCode === code) {
        window.__previewCompareCode = null; // Lock it in
    } else {
        const idx = window.finCompareHospitals.indexOf(code);
        if (idx === -1) window.finCompareHospitals.push(code);
        else window.finCompareHospitals.splice(idx, 1);
    }
    updateFinancialChart();
    renderFinComparePanel();
};

function renderFinComparePanel() {
    const panel = document.getElementById('finComparePanel');
    if (!panel) return;
    const primaryCode = window.currentFinHospitalCode;
    const others = comparisonList.filter(c => c.code !== primaryCode);
    if (others.length === 0) { panel.style.display = 'none'; return; }
    panel.style.display = 'flex';
    panel.innerHTML = others.map(h => {
        const active = window.finCompareHospitals.includes(h.code);
        const colors = ['#f97316', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16', '#a78bfa'];
        const cIdx = comparisonList.findIndex(c => c.code === h.code) % colors.length;
        const baseColor = colors[cIdx];
        return `<span onclick="window.toggleFinCompare('${h.code}')" onmouseenter="if(window.highlightFinCompare) window.highlightFinCompare('${h.code}')" onmouseleave="if(window.clearFinCompareHighlight) window.clearFinCompareHighlight()" style="cursor:pointer; padding:4px 10px; border-radius:20px; font-size:0.75rem; font-weight:600; border:2px solid ${baseColor}; background:${active ? baseColor : 'transparent'}; color:${active ? '#fff' : baseColor}; transition:all 0.2s; white-space:nowrap; user-select:none;">${h.name}</span>`;
    }).join('');
}

window.buildFinDataForHospital = function (hospital) {
    if (!hospital) return [];

    // Fallback if data not attached for some reason
    const allRows = Object.values(hospital.mgmtData || {});
    if (allRows.length === 0) return [];
    ...
    const rowObjects = allRows.map(row => {
        ...
        return {
            ...
            R_InterUnit: getVal('A9001010'),
            R_UC_Total: getVal('A410S'),
            R_OpRevenue: getVal('A41000'),

            // UC Detail breakdown
            R_UC_OP_Capitation: getVal('A4100001'),
            R_UC_OP_Net: getVal('A4101040'),
            R_UC_OP_Gross: getVal('A4101010'),
            R_UC_OP_Diff: getVal('A4101020'),
            R_UC_OP_Salary: getVal('A4101021'),

            R_UC_PP_Capitation: getVal('A4101040.1'),
            R_UC_PP_Net: getVal('A4101043'),
            R_UC_PP_Gross: getVal('A4101041'),
            R_UC_PP_Diff: getVal('A4101042'),
            R_UC_PP_Salary: getVal('A4101042.1'),
            ...
            R_UC_Area: getVal('A4103040'),
            R_UC_PP_Other: getVal('A4103050'),
            R_UC_Specific: getVal('A4103060'),
            R_UC_Performance: getVal('A4103070'),
            R_UC_Other_Misc: getVal('A4103080'),
            R_UC_CF: getVal('A4103100'),
            R_UC_BadDebt_Writeoff: getVal('A4103101'),
...
function openFinancialChartModal(hospitalCode) {
    ...
    const rowObjects = buildFinDataForHospital(hospital);
    ...
    window.finSubGroupVisibility = null;
    window.finSubItemState = null;
    window.finStackState = null;

    setupFinancialChartModal();
    loadServiceData(hospital);
...
window.finNonCumulative = false; // Non-cumulative quarterly values toggle
...
function setupFinancialChartModal() {
    ...
    if (!modal) {
        const modalHTML = `
                <div id="finChartModal" class="modal" style="z-index: 4100; display: block;">
                    <div class="modal-content" style="max-width:1400px; width:95%; height:95vh; display: flex; flex-direction: column;">
                        <div class="modal-header" style="padding: 10px 20px; flex-direction: column; align-items: stretch; gap: 8px;">
                            <div style="display: flex; align-items: center; justify-content: space-between; padding-bottom: 4px; border-bottom: 1px solid #e2e8f0; margin-bottom: 4px;">
                                <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap; flex: 1;">
                                    <h2 style="font-size:1.1rem; margin:0; white-space:nowrap; display: flex; align-items: center; gap: 8px;">
                                        โครงสร้างทางการเงิน: 
                                        <select id="finHospitalSelect" onchange="window.updateFinHospital(this.value)" onmouseenter="if(window.highlightFinPrimary) window.highlightFinPrimary()" onmouseleave="if(window.clearFinCompareHighlight) window.clearFinCompareHighlight()" style="padding: 2px 8px; font-size: 1rem; border-radius: 6px; border: 1px solid #cbd5e1; background: #eff6ff; font-weight: 600; color: #1d4ed8; outline: none; cursor: pointer;">
                                            <!-- Options populated dynamically -->
                                        </select>
                                    </h2>
                                    <span style="font-size: 0.9rem; font-weight: 600; color: #475569; white-space: nowrap; margin-left: 10px;">เปรียบเทียบกับ:</span>
                                    <div id="finComparePanel" style="display: flex; flex-wrap: wrap; gap: 6px; flex: 1;">
                                        <!-- Chips generated by renderFinComparePanel -->
                                    </div>
                                </div>
                                <span class="close-modal" onclick="closeFinancialChartModal()" style="font-size:24px; cursor:pointer; padding:5px; margin-left:10px;">&times;</span>
                            </div>
                        </div>
                        ...
                        <div id="finChartControls" class="modal-controls">
                             ...
                             <div class="control-group">
                                <select id="finStartYear" onchange="window.updateFinFilters()" class="control-select" style="min-width: 0; width: 65px; padding: 4px;"></select>
                                <span style="color:#64748b;">-</span>
                                <select id="finEndYear" onchange="window.updateFinFilters()" class="control-select" style="min-width: 0; width: 65px; padding: 4px;"></select>
                                <select id="finQuarter" onchange="window.updateFinFilters()" class="control-select" style="min-width: 0; width: 110px; padding: 4px;">
                                    <option value="All">ทุกไตรมาส (Q1-Q4)</option>
                                    <option value="Yearly">รายปี (รวมทั้งปี)</option>
                                    <option value="1">Q1 (ต.ค. - ธ.ค.)</option>
                                    <option value="2">Q2 (ม.ค. - มี.ค.)</option>
                                    <option value="3">Q3 (เม.ย. - มิ.ย.)</option>
                                    <option value="4">Q4 (ก.ค. - ก.ย.)</option>
                                </select>
                             </div>
                             ...
                             <div class="control-group" style="display:flex; align-items:center; gap:8px;" onclick="document.getElementById('finSortToggle').click()">
                                <label class="toggle-switch" style="transform:scale(0.8); transform-origin:left center;" onclick="event.stopPropagation()">
                             ...
                             <div class="control-group" style="display:flex; align-items:center; gap:8px;" onclick="document.getElementById('finNonCumulativeToggle').click()">
                                <label class="toggle-switch" style="transform:scale(0.8); transform-origin:left center;" onclick="event.stopPropagation()">
                                    <input type="checkbox" id="finNonCumulativeToggle" onchange="window.finNonCumulative = this.checked; updateFinancialChart();">
                                    <span class="slider"></span>
                                </label>
                                <span class="control-label">รายไตรมาส (ไม่สะสม)</span>
                             </div>

                             <div class="control-group" style="display:flex; align-items:center; gap:8px;" onclick="document.getElementById('finChartTypeToggle').click()">
                                <label class="toggle-switch" style="transform:scale(0.8); transform-origin:left center;" onclick="event.stopPropagation()">
                                    <input type="checkbox" id="finChartTypeToggle" onchange="window.finChartTypeLine = this.checked; updateFinancialChart();">
                                    <span class="slider"></span>
                                </label>
                                <span class="control-label">กราฟเส้น</span>
                             </div>

                             <div id="finAreaToggleGroup" class="control-group" style="display:flex; align-items:center; gap:8px; display: none;" onclick="document.getElementById('finAreaChartToggle').click()">
                                <label class="toggle-switch" style="transform:scale(0.8); transform-origin:left center;" onclick="event.stopPropagation()">
                                    <input type="checkbox" id="finAreaChartToggle" onchange="window.finAreaChart = this.checked; updateFinancialChart();">
                                    <span class="slider"></span>
                                </label>
                                <span class="control-label">กราฟพื้นที่ (สะสม)</span>
                             </div>
                        </div>
    ...
    // Update Primary Hospital Title (Dropdown style)
    const hospSel = document.getElementById('finHospitalSelect');
    if (hospSel) {
        hospSel.innerHTML = comparisonList.map(c =>
            `<option value="${c.code}" ${c.code === window.currentFinHospitalCode ? 'selected' : ''}>${c.name}</option>`
        ).join('');
    }

    // Render compare panel correctly
    renderFinComparePanel();
    ...
    if (document.getElementById('finNonCumulativeToggle')) document.getElementById('finNonCumulativeToggle').checked = window.finNonCumulative;
    if (document.getElementById('finChartTypeToggle')) document.getElementById('finChartTypeToggle').checked = !!window.finChartTypeLine;
    if (document.getElementById('finAreaChartToggle')) document.getElementById('finAreaChartToggle').checked = !!window.finAreaChart;
    if (document.getElementById('finAreaToggleGroup')) document.getElementById('finAreaToggleGroup').style.display = window.finChartTypeLine ? 'flex' : 'none';
...
window.FIN_STRUCTURE = [
        ...
        subgroups: [
            {
                ...
                items: [
                    ...
                    //{ key: 'L_Trade_Lab_GRIR', label: 'เจ้าหนี้การค้า GR/IR ยาและเวชภัณฑ์มิใช่ยา', accessor: d => d.L_Trade_Lab_GRIR, color: '#991b1b', border: '#7f1d1d' },
    ...
    {
        id: 'Revenue', label: 'รายได้ (Revenue)', color: '#10b981',
        subgroups: [
            /* {
                 id: 'OpRevenueGroup', label: 'รายได้จากการดำเนินงาน',
                 summary: { label: 'รายได้จากการดำเนินงาน (A41000)', accessor: d => d.R_OpRevenue, color: '#10b981', border: '#059669' },
                 items: [
                     { key: 'R_OpRevenue', label: 'รายได้จากการดำเนินงานรวม', accessor: d => d.R_OpRevenue, color: '#10b981', border: '#059669' }
                 ]
             },  */
            {
                ...
                items: [
                    {
                        key: 'R_UC_OP_Net', label: 'รายได้ UC-OP ใน CUP สุทธิ', accessor: d => d.R_UC_OP_Net, color: '#064e3b', border: '#065f46',
                        subItems: [
                            { key: 'R_UC_OP_Capitation', label: 'รายได้จากกองทุน UC-OP เหมาจ่าย', accessor: d => d.R_UC_OP_Capitation, color: '#10b981', border: '#059669', hideInChart: true },
                            { key: 'R_UC_OP_Gross', label: 'รายได้ UC-OP ใน CUP (ก่อนหัก)', accessor: d => d.R_UC_OP_Gross, color: '#065f46', border: '#047857', hideInChart: true },
                            { key: 'R_UC_OP_Diff', label: 'หัก ส่วนต่างค่ารักษา UC-OP', accessor: d => d.R_UC_OP_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true },
                            { key: 'R_UC_OP_Salary', label: 'หัก ส่วนปรับลดค่าแรง OP', accessor: d => d.R_UC_OP_Salary, color: '#ef4444', border: '#dc2626', hideInChart: true }
                        ]
                    },
                    {
                        key: 'R_UC_PP_Net', label: 'รายได้ UC-P&P สุทธิ', accessor: d => d.R_UC_PP_Net, color: '#059669', border: '#047857',
                        subItems: [
                            { key: 'R_UC_PP_Capitation', label: 'รายได้จากกองทุน UC-P&P เหมาจ่าย', accessor: d => d.R_UC_PP_Capitation, color: '#10b981', border: '#059669', hideInChart: true },
                            { key: 'R_UC_PP_Gross', label: 'รายได้ UC-P&P (ก่อนหัก)', accessor: d => d.R_UC_PP_Gross, color: '#10b981', border: '#059669', hideInChart: true },
                            { key: 'R_UC_PP_Diff', label: 'หัก ส่วนต่างค่ารักษา UC-P&P', accessor: d => d.R_UC_PP_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true },
                            { key: 'R_UC_PP_Salary', label: 'หัก ส่วนปรับลดค่าแรง P&P', accessor: d => d.R_UC_PP_Salary, color: '#ef4444', border: '#dc2626', hideInChart: true }
                        ]
                    },
                    {
                        key: 'R_UC_IP_Net', label: 'รายได้ UC-IP สุทธิ', accessor: d => d.R_UC_IP_Net, color: '#10b981', border: '#059669',
                        subItems: [
                            { key: 'R_UC_IP_Gross', label: 'รายได้ UC-IP (ก่อนหัก)', accessor: d => d.R_UC_IP_Gross, color: '#34d399', border: '#10b981', hideInChart: true },
                            { key: 'R_UC_IP_Diff', label: 'หัก ส่วนต่างค่ารักษา UC-IP (DRG)', accessor: d => d.R_UC_IP_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true },
                            { key: 'R_UC_IP_Salary', label: 'หัก ส่วนปรับลดค่าแรง IP', accessor: d => d.R_UC_IP_Salary, color: '#ef4444', border: '#dc2626', hideInChart: true },
                            { key: 'R_UC_IP_Add', label: 'บวก ส่วนต่างค่ารักษา UC-IP', accessor: d => d.R_UC_IP_Add, color: '#10b981', border: '#059669', hideInChart: true }
                        ]
                    },
                    {
                        key: 'R_UC_Refer_Net', label: 'รายได้จากการเรียกเก็บ UC สุทธิ', accessor: d => d.R_UC_Refer_Net, color: '#34d399', border: '#10b981',
                        subItems: [
                            { key: 'R_UC_Refer_Gross', label: 'รายได้จากการเรียกเก็บ UC (ก่อนหัก)', accessor: d => d.R_UC_Refer_Gross, color: '#6ee7b7', border: '#34d399', hideInChart: true },
                            { key: 'R_UC_Refer_Diff', label: 'หัก ส่วนต่างค่ารักษา OP UC', accessor: d => d.R_UC_Refer_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true },
                            { key: 'R_UC_Refer_Add', label: 'บวก ส่วนต่างค่ารักษา OP UC', accessor: d => d.R_UC_Refer_Add, color: '#10b981', border: '#059669', hideInChart: true }
                        ]
                    },
                    {
                        key: 'R_UC_CR_Net', label: 'รายได้ค่ารักษา UC - บริการเฉพาะ (CR)- สุทธิ', accessor: d => d.R_UC_CR_Net, color: '#6ee7b7', border: '#34d399',
                        subItems: [
                            { key: 'R_UC_CR_Gross', label: 'รายได้ UC-CR (ก่อนหัก)', accessor: d => d.R_UC_CR_Gross, color: '#a7f3d0', border: '#6ee7b7', hideInChart: true },
                            { key: 'R_UC_CR_Diff', label: 'หัก ส่วนต่าง UC-CR', accessor: d => d.R_UC_CR_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true },
                            { key: 'R_UC_CR_Add', label: 'บวก ส่วนต่าง UC-CR', accessor: d => d.R_UC_CR_Add, color: '#10b981', border: '#059669', hideInChart: true }
                        ]
                    },
                    { key: 'R_UC_Refer_OP', label: 'รายได้ค่ารักษา OP Refer', accessor: d => d.R_UC_Refer_OP, color: '#a7f3d0', border: '#6ee7b7' },
                    { key: 'R_UC_Area', label: 'รายได้จากกองทุน UC - พื้นที่เฉพาะ', accessor: d => d.R_UC_Area, color: '#ecfdf5', border: '#d1fae5' },
                    { key: 'R_UC_PP_Other', label: 'รายได้กองทุน UC P&P อื่น', accessor: d => d.R_UC_PP_Other, color: '#10b981', border: '#059669' },
                    { key: 'R_UC_Specific', label: 'รายได้จากกองทุน UC เฉพาะโรคอื่น', accessor: d => d.R_UC_Specific, color: '#059669', border: '#047857' },
                    { key: 'R_UC_Performance', label: 'รายได้กองทุน UC- ตามผลงาน', accessor: d => d.R_UC_Performance, color: '#047857', border: '#065f46' },
                    { key: 'R_UC_Other_Misc', label: 'รายได้กองทุน UC- อื่นๆ', accessor: d => d.R_UC_Other_Misc, color: '#065f46', border: '#064e3b' },
                    { key: 'R_UC_CF', label: 'รายได้กองทุน UC-CF', accessor: d => d.R_UC_CF, color: '#064e3b', border: '#022c22' },
                    { key: 'R_UC_BadDebt_Writeoff', label: 'รายได้จากการยกหนี้ (ส่งต่อผู้ป่วย)', accessor: d => d.R_UC_BadDebt_Writeoff, color: '#022c22', border: '#000000' }
                ]
            },
            {
                id: 'CSRevenue', label: 'รวมรายได้เบิกจ่ายตรง',
                summary: { label: 'รวมรายได้เบิกจ่ายตรง (CS Total)', accessor: d => d.R_CS, color: '#3b82f6', border: '#2563eb' },
                items: [
                    { key: 'R_CS_OP', label: 'รายได้เบิกจ่ายตรง OP', accessor: d => d.R_CS_OP, color: '#2563eb', border: '#1d4ed8' },
                    {
                        key: 'R_CS_IP_Net', label: 'รายได้เบิกจ่ายตรง IP สุทธิ', accessor: d => d.R_CS_IP_Net, color: '#1d4ed8', border: '#1e40af',
                        subItems: [
                            { key: 'R_CS_IP_Gross', label: 'รายได้เบิกจ่ายตรง IP (ก่อนหัก)', accessor: d => d.R_CS_IP_Gross, color: '#3b82f6', border: '#2563eb', hideInChart: true },
                            { key: 'R_CS_Diff', label: 'หัก/บวก ส่วนต่างเบิกจ่ายตรง', accessor: d => d.R_CS_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true }
                        ]
                    },
                    { key: 'R_CS_Employer', label: 'รายได้ค่ารักษาเบิกต้นสังกัด', accessor: d => d.R_CS_Employer, color: '#60a5fa', border: '#3b82f6' }
                ]
            },
            {
                id: 'LAORevenue', label: 'รวมรายได้ อปท.',
                summary: { label: 'รวมรายได้ อปท. (LAO Total)', accessor: d => d.R_LAO_Total, color: '#6366f1', border: '#4f46e5' },
                items: [
                    { key: 'R_LAO_OP', label: 'รายได้เบิกจ่ายตรง อปท. OP', accessor: d => d.R_LAO_OP, color: '#4f46e5', border: '#4338ca' },
                    {
                        key: 'R_LAO_IP_Net', label: 'รายได้เบิกจ่ายตรง อปท. IP สุทธิ', accessor: d => d.R_LAO_IP_Net, color: '#4338ca', border: '#3730a3',
                        subItems: [
                            { key: 'R_LAO_IP_Gross', label: 'รายได้เบิกจ่ายตรง อปท. IP (ก่อนหัก)', accessor: d => d.R_LAO_IP_Gross, color: '#6366f1', border: '#4f46e5', hideInChart: true },
                            { key: 'R_LAO_Diff', label: 'หัก/บวก ส่วนต่าง อปท.', accessor: d => d.R_LAO_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true }
                        ]
                    }
                ]
            },
            {
                id: 'SSRevenue', label: 'รวมรายได้ประกันสังคม',
                summary: { label: 'รวมรายได้ประกันสังคม (SS Total)', accessor: d => d.R_SS, color: '#8b5cf6', border: '#7c3aed' },
                items: [
                    {
                        key: 'R_SS_Network_Net', label: 'รายได้ประกันสังคมในเครือข่าย สุทธิ', accessor: d => d.R_SS_Network_Net, color: '#7c3aed', border: '#6d28d9',
                        subItems: [
                            { key: 'R_SS_Network_Gross', label: 'รายได้ประกันสังคมในเครือข่าย (ก่อนหัก)', accessor: d => d.R_SS_Network_Gross, color: '#8b5cf6', border: '#7c3aed', hideInChart: true },
                            { key: 'R_SS_Network_Diff', label: 'หัก/บวก ส่วนต่างประกันสังคม', accessor: d => d.R_SS_Network_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true }
                        ]
                    },
                    { key: 'R_SS_NonNetwork', label: 'รายได้ประกันสังคมนอกเครือข่าย', accessor: d => d.R_SS_NonNetwork, color: '#a78bfa', border: '#8b5cf6' },
                    { key: 'R_SS_Fund', label: 'รายได้กองทุนประกันสังคม', accessor: d => d.R_SS_Fund, color: '#c4b5fd', border: '#a78bfa' },
                    { key: 'R_SS_Other_Misc', label: 'รายได้ค่าตอบแทน/พัฒนากิจการ', accessor: d => d.R_SS_Other_Misc, color: '#ddd6fe', border: '#c4b5fd' }
                ]
            },
            {
                id: 'AlienRevenue', label: 'รวมรายได้แรงงานต่างด้าว',
                summary: { label: 'รวมรายได้แรงงานต่างด้าว (Alien Total)', accessor: d => d.R_Alien_Total, color: '#f59e0b', border: '#d97706' },
                items: [
                    {
                        key: 'R_Alien_Net', label: 'รายได้แรงงานต่างด้าวใน CUP สุทธิ', accessor: d => d.R_Alien_Net, color: '#d97706', border: '#b45309',
                        subItems: [
                            { key: 'R_Alien_Gross', label: 'รายได้แรงงานต่างด้าวใน CUP (ก่อนหัก)', accessor: d => d.R_Alien_Gross, color: '#f59e0b', border: '#d97706', hideInChart: true },
                            { key: 'R_Alien_Diff', label: 'หัก/บวก ส่วนต่างต่างด้าว', accessor: d => d.R_Alien_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true }
                        ]
                    },
                    { key: 'R_Alien_NonCUP', label: 'รายได้แรงงานต่างด้าวนอก CUP', accessor: d => d.R_Alien_NonCUP, color: '#fbbf24', border: '#f59e0b' },
                    { key: 'R_Alien_Fund', label: 'รายได้กองทุนแรงงานต่างด้าว', accessor: d => d.R_Alien_Fund, color: '#fcd34d', border: '#fbbf24' },
                    { key: 'R_Alien_Checkup', label: 'รายได้ค่าตรวจสุขภาพต่างด้าว', accessor: d => d.R_Alien_Checkup, color: '#fde68a', border: '#fcd34d' }
                ]
            },
            {
                id: 'StatusRevenue', label: 'รวมรายได้บุคคลสิทธิฯ',
                summary: { label: 'รวมรายได้บุคคลสิทธิฯ (Status Total)', accessor: d => d.R_Status_Total, color: '#ec4899', border: '#db2777' },
                items: [
                    {
                        key: 'R_Status_NonCUP_Net', label: 'รายได้บุคคลสิทธินอก CUP สุทธิ', accessor: d => d.R_Status_NonCUP_Net, color: '#db2777', border: '#be185d',
                        subItems: [
                            { key: 'R_Status_NonCUP_Gross', label: 'รายได้บุคคลสิทธินอก CUP (ก่อนหัก)', accessor: d => d.R_Status_NonCUP_Gross, color: '#ec4899', border: '#db2777', hideInChart: true },
                            { key: 'R_Status_NonCUP_Diff', label: 'หัก/บวก ส่วนต่างบุคคลสิทธิฯ', accessor: d => d.R_Status_NonCUP_Diff, color: '#dc2626', border: '#991b1b', hideInChart: true }
                        ]
                    },
                    { key: 'R_Status_InCUP', label: 'รายได้บุคคลสิทธิ OP ใน CUP', accessor: d => d.R_Status_InCUP, color: '#f472b6', border: '#ec4899' },
                    { key: 'R_Status_Subsidy', label: 'เงินอุดหนุนเหมาจ่ายบุคคลสิทธิฯ', accessor: d => d.R_Status_Subsidy, color: '#f9a8d4', border: '#f472b6' }
                ]
            },
            {
                id: 'OtherMedicalRevenue', label: 'รายได้ค่ารักษาอื่นๆ',
                summary: { label: 'รายได้ค่ารักษาอื่นๆ (Other Med)', accessor: d => d.R_Medical - (d.R_UC_Total + d.R_CS + d.R_SS + d.R_LAO_Total + d.R_Alien_Total + d.R_Status_Total), color: '#64748b', border: '#475569' },
                items: [
                    { key: 'R_Med_Act', label: 'รายได้ค่ารักษาจาก พรบ.', accessor: d => d.R_Med_Act, color: '#475569', border: '#334155' },
                    { key: 'R_Med_EMS', label: 'รายได้จากระบบ EMS', accessor: d => d.R_Med_EMS, color: '#94a3b8', border: '#64748b' },
                    { key: 'R_Med_Other_Service', label: 'รายได้ค่าบริการอื่นๆ', accessor: d => d.R_Med_Other_Service, color: '#cbd5e1', border: '#94a3b8' },
                    { key: 'R_Med_Other_Misc', label: 'รายได้ค่ารักษาพยาบาลอื่นๆ', accessor: d => d.R_Med_Other_Misc, color: '#e2e8f0', border: '#cbd5e1' }
                ]
            },
            {
                ...
                items: [
                    { key: 'R_Personnel', label: 'งบประมาณส่วนบุคลากร', accessor: d => d.R_Personnel, color: '#15803d', border: '#14532d' },
                    { key: 'R_Budget_Subsidy', label: 'รายได้จากงบประมาณแผ่นดิน-เงินอุดหนุน', accessor: d => d.R_Budget_Subsidy, color: '#16a34a', border: '#15803d' },
                    { key: 'R_Budget_Others', label: 'รายได้จากงบประมาณแผ่นดิน-อื่นๆ', accessor: d => d.R_Budget_Others, color: '#22c55e', border: '#16a34a' },
                    { key: 'R_Budget_Capital', label: 'รายได้จากงบประมาณแผ่นดิน-งบลงทุน', accessor: d => d.R_Budget_Capital, color: '#0d9488', border: '#0f766e' },
                    { key: 'R_UC_Capital', label: 'รายได้งบลงทุน UC', accessor: d => d.R_UC_Capital, color: '#14b8a6', border: '#0d9488' },
                    { key: 'R_InterAgency_Support', label: 'รายได้ช่วยเหลือจากหน่วยงานอื่น', accessor: d => d.R_InterAgency_Support, color: '#06b6d4', border: '#0891b2' },
                    { key: 'R_Donation', label: 'รายได้จากการรับบริจาค', accessor: d => d.R_Donation, color: '#2dd4bf', border: '#14b8a6' },
                    { key: 'R_Other_Fees', label: 'รายได้ค่าธรรมเนียม ฯลฯ', accessor: d => d.R_Other_Fees, color: '#5eead4', border: '#2dd4bf' },
                    { key: 'R_InterUnit', label: 'รายได้ระหว่างหน่วยงาน (หน่วยเบิกจ่าย)', accessor: d => d.R_InterUnit, color: '#94a3b8', border: '#64748b' },
                    {
                        key: 'R_OtherGeneral',
                        label: 'รายได้อื่นๆ (คงเหลือ)',
                        accessor: d => d.R_OtherGeneral - (d.R_InterAgency_Support + d.R_Budget_Capital + d.R_UC_Capital + d.R_Budget_Others + d.R_Donation + d.R_Other_Fees + d.R_Budget_Subsidy + d.R_InterUnit),
                        color: '#cbd5e1',
                        border: '#94a3b8'
                    }
                ]
        ...
        subgroups: [
            {
                ...
                items: [
                    { key: 'E_Med_Labour', label: 'ค่าแรงบริการ (เงินเดือน/ค่าจ้าง/ค่าตอบแทน)', accessor: d => d.E_Med_Labour, color: '#78350f', border: '#451a03' },
                    { key: 'E_Med_Material', label: 'ค่าของบริการ (ยา/เวชภัณฑ์/วัสดุ)', accessor: d => d.E_Med_Material, color: '#92400e', border: '#78350f' },
                    { key: 'E_Med_Depre', label: 'ค่าเสื่อมบริการ', accessor: d => d.E_Med_Depre, color: '#b45309', border: '#92400e' },

                    { key: 'E_Med_Refer_UC', label: 'ค่ารักษาตามจ่าย UC', accessor: d => d.E_Med_Refer_UC, color: '#d97706', border: '#b45309' },
                    { key: 'E_Med_Refer_SP', label: 'ค่ารักษาตามจ่ายในสังกัด สป', accessor: d => d.E_Med_Refer_SP, color: '#f59e0b', border: '#d97706' },
                    { key: 'E_Med_Refer_OtherSP', label: 'ค่ารักษาตามจ่ายต่างสังกัด สป', accessor: d => d.E_Med_Refer_OtherSP, color: '#fbbf24', border: '#f59e0b' },
                    { key: 'E_Med_Refer_Alien', label: 'ค่ารักษาตามจ่ายแรงงานต่างด้าว', accessor: d => d.E_Med_Refer_Alien, color: '#fcd34d', border: '#fbbf24' },
                    { key: 'E_Med_Refer_Status', label: 'ค่ารักษาตามจ่ายบุคคลสิทธิฯ', accessor: d => d.E_Med_Refer_Status, color: '#fde68a', border: '#fcd34d' },
                    { key: 'E_Med_Lab_Service', label: 'ค่าจ้างตรวจทางห้องปฏิบัติการ', accessor: d => d.E_Med_Lab_Service, color: '#451a03', border: '#000000' },

                    { key: 'E_Med_Other', label: 'ค่าใช้สอยบริการอื่น (สัดส่วน 70)', accessor: d => Math.max(0, d.E_Med_Total - (d.E_Med_Labour + d.E_Med_Material + d.E_Med_Depre + d.E_Med_Refer_UC + d.E_Med_Refer_SP + d.E_Med_Refer_OtherSP + d.E_Med_Refer_Alien + d.E_Med_Refer_Status + d.E_Med_Lab_Service)), color: '#d97706', border: '#b45309' }
                ]
            },
            {
                ...
                items: [
                    { key: 'E_Op_Labour', label: 'ค่าแรงสนับสนุน (เงินเดือน/ค่าจ้าง/ค่าตอบแทน)', accessor: d => d.E_Op_Labour, color: '#d97706', border: '#b45309' },
                    { key: 'E_Op_Material', label: 'ค่าของสนับสนุน (สัดส่วน 30)', accessor: d => d.E_Op_Material, color: '#fbbf24', border: '#d97706' },
                    { key: 'E_Op_Depre', label: 'ค่าเสื่อมสนับสนุน', accessor: d => d.E_Op_Depre, color: '#fcd34d', border: '#fbbf24' },
                    { key: 'E_Op_InterUnit', label: 'ค่าใช้จ่ายระหว่างหน่วยงาน (หน่วยเบิกจ่าย)', accessor: d => d.E_Op_InterUnit, color: '#e9c03bda', border: '#fbbf24' },
                    { key: 'E_Op_InterUnit_Other', label: 'ค่าใช้จ่ายอื่น-ค่าใช้จ่ายระหว่างหน่วยงาน', accessor: d => d.E_Op_InterUnit_Other, color: '#fcd34d', border: '#fbbf24' },
                    { key: 'E_Op_BadDebt_Provision', label: 'หนี้สงสัยจะสูญ', accessor: d => d.E_Op_BadDebt_Provision, color: '#991b1b', border: '#7f1d1d' },
                    { key: 'E_Op_BadDebt_Actual', label: 'หนี้สูญ', accessor: d => d.E_Op_BadDebt_Actual, color: '#7f1d1d', border: '#450a0a' },
                    { key: 'E_Op_Other_General', label: 'ค่าใช้จ่ายในการดำเนินงานอื่นๆ', accessor: d => d.E_Op_Other_General, color: '#fef3c7', border: '#fde68a' },
                    { key: 'E_Op_Other', label: 'ค่าใช้สอยสนับสนุนอื่นๆ', accessor: d => Math.max(0, d.E_Op_Total - (d.E_Op_Labour + d.E_Op_Material + d.E_Op_Depre + d.E_Op_InterUnit + d.E_Op_InterUnit_Other + d.E_Op_BadDebt_Provision + d.E_Op_BadDebt_Actual + d.E_Op_Other_General)), color: '#fde68a', border: '#fcd34d' }
                ]
            },
            {
                ...
                items: [
                    { key: 'E_Other_Project', label: 'ค่าใช้จ่ายโครงการ', accessor: d => d.E_Other_Project, color: '#9a3412', border: '#7c2d12' },
                    { key: 'E_Other_Project_PP', label: 'ค่าใช้จ่ายโครงการ PP', accessor: d => d.E_Other_Project_PP, color: '#c2410c', border: '#9a3412' },
                    { key: 'E_Other_Refund', label: 'คืนเงินค่ารักษา/อุปกรณ์/อวัยวะเทียม', accessor: d => d.E_Other_Refund, color: '#ea580c', border: '#c2410c' },
                    { key: 'E_Other_Maintenance', label: 'ค่าจ้าง/ค่าเช่า/ซ่อมบำรุง (งบลงทุน UC)', accessor: d => d.E_Other_Maintenance, color: '#f97316', border: '#ea580c' },
                    { key: 'E_Other', label: 'ค่าใช้จ่ายอื่นๆ (คงเหลือ)', accessor: d => Math.max(0, d.E_Other - (d.E_Other_Project + d.E_Other_Project_PP + d.E_Other_Refund + d.E_Other_Maintenance)), color: '#9a3412', border: '#7c2d12' }
                ]
            }
        ]
    },
    {
        id: 'ServicesYearly', label: 'สถิติบริการรายปี (Yearly Services)', color: '#8b5cf6', _displayOrder: 101,
        subgroups: [
            {
                id: 'OPDRightPerson', label: 'จำนวนรับบริการ OPD (คน/ปี)',
                summary: { label: 'จำนวนรับบริการ OPD (คน/ปี)', accessor: d => (d.OPD_R_UC_P || 0) + (d.OPD_R_Civil_P || 0) + (d.OPD_R_SS_P || 0) + (d.OPD_R_Alien_P || 0) + (d.OPD_R_Other_P || 0), color: '#818cf8', border: '#6366f1' },
                items: [
            ...
            {
                id: 'OPDRightVisits', label: 'จำนวนรับบริการ OPD (ครั้ง/ปี)',
                summary: { label: 'จำนวนรับบริการ OPD (ครั้ง/ปี)', accessor: d => (d.OPD_R_UC_V || 0) + (d.OPD_R_Civil_V || 0) + (d.OPD_R_SS_V || 0) + (d.OPD_R_Alien_V || 0) + (d.OPD_R_Other_V || 0), color: '#6366f1', border: '#4f46e5' },
                items: [
            ...
            {
                id: 'IPDRightPerson', label: 'จำนวนผู้รับบริการ IPD (คน/ปี)',
                summary: { label: 'จำนวนผู้รับบริการ IPD (คน/ปี)', accessor: d => (d.IPD_R_UC_P || 0) + (d.IPD_R_Civil_P || 0) + (d.IPD_R_SS_P || 0) + (d.IPD_R_Alien_P || 0) + (d.IPD_R_Other_P || 0), color: '#34d399', border: '#059669' },
                items: [
            ...
            {
                id: 'IPDRightDays', label: 'จำนวนวันนอน IPD (วัน/ปี)',
                summary: { label: 'จำนวนวันนอน IPD (วัน/ปี)', accessor: d => (d.IPD_R_UC_D || 0) + (d.IPD_R_Civil_D || 0) + (d.IPD_R_SS_D || 0) + (d.IPD_R_Alien_D || 0) + (d.IPD_R_Other_D || 0), color: '#10b981', border: '#047857' },
                items: [
    ...
    {
        id: 'Services', label: 'สถิติบริการ (Services)', color: '#6366f1', _displayOrder: 100,
        subgroups: [
            {
                id: 'OPDVisitsTPS', label: 'จำนวนรับบริการ OPD (ครั้ง/ไตรมาส)',
                summary: { label: 'จำนวนรับบริการ OPD (ครั้ง/ไตรมาส)', accessor: d => d.OPD_Visits_TPS || 0, color: '#4f46e5', border: '#4338ca' },
                items: [{ key: 'OPD_Visits_TPS', label: 'จำนวนรับบริการ OPD (ครั้ง/ไตรมาส)', accessor: d => d.OPD_Visits_TPS || 0, color: '#4f46e5', border: '#4338ca' }]
            },
            {
                id: 'IPD_Days_TPS_Group', label: 'จำนวนวันนอน',
                summary: { label: 'จำนวนวันนอน', accessor: d => d.IPD_Days_TPS || 0, color: '#10b981', border: '#047857' },
                items: [{ key: 'IPD_Days_TPS', label: 'จำนวนวันนอน', accessor: d => d.IPD_Days_TPS || 0, color: '#10b981', border: '#047857' }]
            },
            {
                id: 'SumAdjRW_Group', label: 'Sum AdjRW (Sum AdjRW/ไตรมาส)',
                summary: { label: 'Sum AdjRW (Sum AdjRW/ไตรมาส)', accessor: d => d.SumAdjRW || 0, color: '#059669', border: '#047857' },
                items: [{ key: 'SumAdjRW', label: 'Sum AdjRW (Sum AdjRW/ไตรมาส)', accessor: d => d.SumAdjRW || 0, color: '#059669', border: '#047857' }]
            },
            {
                id: 'BedOccupancy_Group', label: 'อัตราครองเตียง (%)',
                summary: { label: 'อัตราครองเตียง (%)', accessor: d => d.BedOccupancy || 0, color: '#f59e0b', border: '#d97706' },
                items: [{ key: 'BedOccupancy', label: 'อัตราครองเตียง (%)', accessor: d => d.BedOccupancy || 0, color: '#f59e0b', border: '#d97706' }]
            },
            {
                id: 'SumAdjRW_Per_Day_Group', label: 'Sum AdjRW/จำนวนวันนอน',
                summary: { label: 'Sum AdjRW/จำนวนวันนอน', accessor: d => d.SumAdjRW_Per_Day || 0, color: '#ec4899', border: '#be185d' },
                items: [{ key: 'SumAdjRW_Per_Day', label: 'Sum AdjRW/จำนวนวันนอน', accessor: d => d.SumAdjRW_Per_Day || 0, color: '#ec4899', border: '#be185d' }]
            }
        ]
    }
];
...
function updateFinancialChart() {
    ...
    if (!window.globalFinSubItemState) {
        ...
        window.FIN_STRUCTURE.forEach(g => g.subgroups.forEach(sg => sg.items.forEach(item => {
            // Default OFF for Services-type groups, ON for others
            if (g.id === 'Services' || g.id === 'ServicesYearly') {
                window.globalFinSubItemState[item.key] = false;
            } else {
    ...
    if (!window.globalFinStackState) {
        ...
        window.FIN_STRUCTURE.forEach(g => {
            // Only 'Assets' is TRUE by default; 'Services' is now FALSE (closed) by default as requested.
            window.globalFinStackState[g.id] = (g.id === 'Assets');
        });
    }

    if (window.finAreaChart === undefined) window.finAreaChart = false;
    ...
    if (!window.globalFinSubGroupVisibility) {
        ...
        window.FIN_STRUCTURE.forEach(g => g.subgroups.forEach(sg => {
            if (g.id === 'Services' || g.id === 'ServicesYearly') {
                // Services groups: all items hidden by default as requested
                window.globalFinSubGroupVisibility[sg.id] = false;
            } else {
    ...
    if (allRows) {
        ...
        // --- QUARTERLY DIFFERENCING LOGIC ---
        // Some items (OPD Visits, Sum AdjRW) must always be shown as non-cumulative quarterly values.
        // Financial items (Revenue, Expense) are differenced only if window.finNonCumulative is true.
        if (processedRows.length > 0) {
            ...
            Object.keys(rowsByYear).sort().forEach(year => {
                const yearRows = rowsByYear[year].sort((a, b) => parseInt(a.quarter) - parseInt(b.quarter));
                yearRows.forEach((r, idx) => {
                    const newRow = { ...r };
                    if (idx > 0) {
                        const prevRow = yearRows[idx - 1];
                        Object.keys(newRow).forEach(key => {
                            const isAutoNonCum = (key === 'SumAdjRW' || key === 'OPD_Visits_TPS');
                            const isFinancialNonCum = window.finNonCumulative && (key.startsWith('R_') || key.startsWith('E_'));

                            if (isAutoNonCum || isFinancialNonCum) {
                                newRow[key] = (r[key] || 0) - (prevRow[key] || 0);
                            }
                        });
                    }
                    // Calculate Ratio after differencing
                    newRow.SumAdjRW_Per_Day = newRow.IPD_Days_TPS > 0 ? (newRow.SumAdjRW / newRow.IPD_Days_TPS) : 0;
                    newProcessedRows.push(newRow);
                });
            });
            processedRows = newProcessedRows;
        }

        filteredData = processedRows.filter(r => {
            const y = parseInt(r.year);
            if (y < filter.startYear || y > filter.endYear) return false;
            if (filter.quarter !== 'All' && filter.quarter !== 'Yearly' && String(r.quarter) !== String(filter.quarter)) return false;
            return true;
        });

        // --- YEARLY AGGREGATION LOGIC ---
        if (filter.quarter === 'Yearly') {
            const yearlyGroups = {};
            filteredData.forEach(r => {
                if (!yearlyGroups[r.year]) {
                    yearlyGroups[r.year] = {
                        ...r,
                        _qCount: 0,
                        _sumFields: ['OPD_Visits_TPS', 'IPD_Days_TPS', 'SumAdjRW'],
                        _avgFields: ['BedOccupancy', 'CMI'],
                        _yearlyFields: [
                            'OPD_R_Civil_P', 'OPD_R_Civil_V', 'OPD_R_SS_P', 'OPD_R_SS_V', 'OPD_R_UC_P', 'OPD_R_UC_V', 'OPD_R_Alien_P', 'OPD_R_Alien_V', 'OPD_R_Other_P', 'OPD_R_Other_V',
                            'IPD_R_Civil_P', 'IPD_R_Civil_D', 'IPD_R_SS_P', 'IPD_R_SS_D', 'IPD_R_UC_P', 'IPD_R_UC_D', 'IPD_R_Alien_P', 'IPD_R_Alien_D', 'IPD_R_Other_P', 'IPD_R_Other_D'
                        ]
                    };
                    // Reset sum fields for fresh start
                    yearlyGroups[r.year]._sumFields.forEach(f => yearlyGroups[r.year][f] = 0);
                    yearlyGroups[r.year]._avgFields.forEach(f => yearlyGroups[r.year][f] = 0);
                }
                const g = yearlyGroups[r.year];
                g._qCount++;
                g._sumFields.forEach(f => g[f] += (r[f] || 0));
                g._avgFields.forEach(f => g[f] += (r[f] || 0));

                // For Yearly Fields, we just take the last available value (which should be the same across all since they are annual)
                g._yearlyFields.forEach(f => g[f] = r[f]);
            });

            filteredData = Object.values(yearlyGroups).map(g => {
                const row = { ...g };
                g._avgFields.forEach(f => row[f] = g._qCount > 0 ? g[f] / g._qCount : 0);
                row.quarter = 'Year';
                return row;
            });
        }
    ...
    const mkDS = (label, dataMap, color, borderColor, stack, hidden, customKey, subgroupId) => ({
        label,
        type: window.finChartTypeLine ? 'line' : 'bar',
        data: rawData.map(dataMap),
        backgroundColor: (window.finChartTypeLine && window.finAreaChart) ? color + '40' : color,
        _originalColor: (window.finChartTypeLine && window.finAreaChart) ? color + '40' : color,
        borderColor: borderColor,
        _originalBorder: borderColor,
        borderWidth: window.finChartTypeLine ? 2 : 1,
        fill: (window.finChartTypeLine && window.finAreaChart),
        stack: stack,
        hidden: hidden,
        _customKey: customKey,
        _subgroupId: subgroupId,
        yAxisID: (stack === 'Services' || subgroupId === 'OPDStats' || subgroupId === 'IPDStats') ?
            (subgroupId === 'BedOccupancy_Group' || customKey === 'BedOccupancy' ? 'y2' : 'y1') : 'y'
    });
    ...
    window.FIN_STRUCTURE.forEach(group => {
        ...
        sortedSubgroups.forEach(subgroup => {
            ...
            const isServicesYearlyGroup = (group.id === 'ServicesYearly');
            const isSvcLike = isServicesGroup || isServicesYearlyGroup;

            if (mode === 'summary') {
                const s = subgroup.summary;
                let currentStack = isSvcLike ? subgroup.id : baseStackId;

                // SMART Y-AXIS ASSIGNMENT:
                // BedOccupancy always y2. 
                // IPD Days also y2 so it doesn't get squashed by large OPD Visit counts on y1.
                let yId = !isSvcLike ? 'y' : 'y1';
                if (isSvcLike) {
                    if (subgroup.id === 'SumAdjRW_Group' || subgroup.id === 'SumAdjRW') {
                        yId = 'y3';
                    } else if (subgroup.id === 'BedOccupancy_Group' || subgroup.id === 'BedOccupancy') {
                        yId = 'y4';
                    } else if (subgroup.id === 'SumAdjRW_Per_Day_Group' || subgroup.id === 'SumAdjRW_Per_Day') {
                        yId = 'y5';
                    } else if (subgroup.id === 'IPD_Days_TPS_Group' || subgroup.id === 'IPD_Days_TPS' || subgroup.id === 'IPDRightDays' || subgroup.id === 'IPDRightDays_Group') {
                        yId = 'y2';
                    }
                }
                ...
                datasets.push({
                    ...
                    backgroundColor: (window.finChartTypeLine && window.finAreaChart) ? s.color + '40' : (window.finChartTypeLine ? s.color + '40' : s.color),
                    _originalColor: (window.finChartTypeLine && window.finAreaChart) ? s.color + '40' : (window.finChartTypeLine ? s.color + '40' : s.color),
                ...
                // Inherit sorting if global setting active
                if (window.finSortByValue && rawData.length > 0) {
                    const lastRow = rawData[rawData.length - 1];
                    const prevRow = rawData.length > 1 ? rawData[rawData.length - 2] : null;
                    const getDiff = (it) => {
                    ...
                    items = [...subgroup.items].sort((a, b) => getDiff(a) - getDiff(b));
                }

                items.forEach(item => {
                    if (item.hideInChart) return;
                    ...
                    let currentStack = isSvcLike ? item.key : baseStackId;

                    let yId = 'y';
                    if (isSvcLike) {
                        if (item.key === 'BedOccupancy' || item.label.includes('%')) {
                            yId = 'y4';
                        } else if (item.key === 'SumAdjRW' || item.label.includes('Sum AdjRW (Sum AdjRW/ไตรมาส)')) {
                            yId = 'y3';
                        } else if (item.key === 'SumAdjRW_Per_Day' || item.label.includes('Sum AdjRW/จำนวนวันนอน')) {
                            yId = 'y5';
                        } else if (item.key.includes('Days') || item.key.includes('_D') || item.label.includes('วันนอน')) {
                            yId = 'y2';
                        } else {
                            yId = 'y1';
                        }
                    ...
                    datasets.push({
                        ...
                        backgroundColor: (window.finChartTypeLine && window.finAreaChart) ? item.color + '40' : (window.finChartTypeLine ? item.color + '40' : item.color),
                        _originalColor: (window.finChartTypeLine && window.finAreaChart) ? item.color + '40' : (window.finChartTypeLine ? item.color + '40' : item.color),
                    ...
                    // Add Sub-items if they are NOT hidden in chart (or if explicitly checked for exclusive view)
                    if (item.subItems) {
                        item.subItems.forEach(si => {
                            const isSiChecked = window.finSubItemState ? (window.finSubItemState[si.key] === true) : false;
                            const isNormallyVisible = !si.hideInChart;
                            
                            // If it's a "table only" item, it ONLY shows if checked.
                            // If it's a normal item, it shows if NOT explicitly unchecked.
                            const shouldShowInList = isNormallyVisible ? (window.finSubItemState ? window.finSubItemState[si.key] !== false : true) : isSiChecked;
                            
                            if (!isNormallyVisible && !isSiChecked) return;

                            datasets.push({
                                type: window.finChartTypeLine ? 'line' : 'bar',
                                label: si.label,
                                data: rawData.map(si.accessor),
                                backgroundColor: (window.finChartTypeLine && window.finAreaChart) ? si.color + '40' : si.color,
                                borderColor: si.border,
                                borderWidth: window.finChartTypeLine ? 2 : 1,
                                stack: currentStack,
                                hidden: !isGroupGenerallyVisible || !shouldShowInList,
                                _customKey: si.key,
                                _subgroupId: subgroup.id,
                                yAxisID: yId
                            });
    ...
    // === COMPARISON HOSPITALS: Add line datasets for each compared hospital ===
    const COMPARE_COLORS = ['#f97316', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16', '#a78bfa'];
    (window.finCompareHospitals || []).forEach((compareCode, ci) => {
        const compareHosp = comparisonList.find(c => c.code === compareCode);
        if (!compareHosp || !compareHosp.mgmtData) return;

        // Build and filter data the same way as primary
        let cmpRows = buildFinDataForHospital(compareHosp);
        if (cmpRows.length === 0) return;

        // Apply differencing
        const cmpRowsByYear = {};
        cmpRows.forEach(r => { const y = parseInt(r.year); if (!cmpRowsByYear[y]) cmpRowsByYear[y] = []; cmpRowsByYear[y].push(r); });
        const cmpProcessed = [];
        Object.keys(cmpRowsByYear).sort().forEach(year => {
            const yr = cmpRowsByYear[year].sort((a, b) => parseInt(a.quarter) - parseInt(b.quarter));
            yr.forEach((r, idx) => {
                const nr = { ...r };
                if (idx > 0) {
                    const pr = yr[idx - 1];
                    ['SumAdjRW', 'OPD_Visits_TPS'].forEach(k => { nr[k] = (r[k] || 0) - (pr[k] || 0); });
                    if (window.finNonCumulative) {
                        Object.keys(nr).forEach(k => { if (k.startsWith('R_') || k.startsWith('E_')) nr[k] = (r[k] || 0) - (pr[k] || 0); });
                    }
                }
                nr.SumAdjRW_Per_Day = nr.IPD_Days_TPS > 0 ? (nr.SumAdjRW / nr.IPD_Days_TPS) : 0;
                cmpProcessed.push(nr);
            });
        });

        let cmpFiltered = cmpProcessed.filter(r => {
            const y = parseInt(r.year);
            if (y < filter.startYear || y > filter.endYear) return false;
            if (filter.quarter !== 'All' && filter.quarter !== 'Yearly' && String(r.quarter) !== String(filter.quarter)) return false;
            return true;
        });
        cmpFiltered.sort((a, b) => { const dy = a.year - b.year; if (dy !== 0) return dy; return a.quarter - b.quarter; });

        const colorsDict = ['#f97316', '#8b5cf6', '#06b6d4', '#ec4899', '#84cc16', '#a78bfa'];
        const hIdx = comparisonList.findIndex(c => c.code === compareCode);
        const baseColor = colorsDict[hIdx % colorsDict.length];

        // For each visible primary dataset, add a matching line dataset
        const seenKeys = new Set();
        datasets.forEach(ds => {
            if (ds.hidden) return;
            const key = ds._customKey;
            if (!key || seenKeys.has(key)) return;
            seenKeys.add(key);

            // Find accessor from FIN_STRUCTURE
            let accessor = null;
            for (const grp of window.FIN_STRUCTURE) {
                for (const sg of grp.subgroups) {
                    if (sg.id === key) { accessor = sg.summary.accessor; break; }
                    const item = sg.items.find(it => it.key === key);
                    if (item) { accessor = item.accessor; break; }
                }
                if (accessor) break;
            }
            if (!accessor) return;

            const cmpData = labels.map(lbl => {
                const match = cmpFiltered.find(r => {
                    if (filter.quarter === 'All') return (r.year + ` Q${r.quarter}`) === lbl;
                    if (filter.quarter === 'Yearly') return (r.year + ' (ทั้งปี)') === lbl;
                    return String(r.year) === String(lbl);
                });
                return match ? accessor(match) : null;
            });

            datasets.push({
                label: `${compareHosp.name} - ${ds.label}`,
                data: cmpData,
                ...
                fill: false,
                tension: 0.3,
                stack: 'compare_line_' + key + '_' + compareCode,
                hidden: false,
                _customKey: key + '_cmp_' + compareCode,
                _subgroupId: ds._subgroupId,
                yAxisID: ds.yAxisID,
                _isCmp: true,
                _cmpColor: baseColor
            });
    ...
    // Custom Plugin: draws value and diff labels on top of bars / line points
    const finLabelsPlugin = {
        id: 'finLabelsPlugin',
        afterDatasetsDraw(chart, args, options) {
            // Wait until chart animation finishes before drawing labels
            if (!window._finLabelsReady) return;
            ...
            chart.data.datasets.forEach((dataset, i) => {
                ...
                meta.data.forEach((element, index) => {
                    ...
                    // For bars: getProps returns y=top-of-segment, base=bottom-of-segment
                    // already accounting for stacking — do NOT recompute from scale.
                    // For lines: y is the pixel for this data point.
                    const props = element.getProps(['x', 'y', 'base', 'width', 'height'], true);
                    const x = props.x;

                    // Skip elements whose x is outside the visible chart area (zoom/pan)
                    if (x < chartArea.left || x > chartArea.right) return;

                    const isLine = dataset.type === 'line' || chart.config.type === 'line';
                    const height = isLine ? 20 : props.height;
                    const width  = isLine ? 40 : props.width;

                    // centerY:
                    //   line → use props.y (Chart.js rendered position, animation-complete-safe)
                    //           offset 10px above the point for readability
                    //   bar  → centre inside the stacked segment using props.y & props.base
                    // NO stagger: stagger causes misalignment when multiple hospitals are shown
                    let centerY;
                    if (isLine) {
                        // props.y is the exact pixel the data point is drawn at (we wait for animation)
                        centerY = props.y - 10;
                    } else {
                        // props.y = top pixel of this segment (cumulative-stack-aware)
                        // props.base = bottom pixel of this segment
                        centerY = (props.y + props.base) / 2;
                    }
                    ...
                    if (isLine) {
                        ...
                        ctx.shadowBlur = 4;
                        ctx.fillStyle = dataset.borderColor || '#333333';
                    } else {
                    ...
                    if (window.showFinValues && height > 15) {
                        const fontSize = Math.min(11, isLine ? 11 : width / 4.5);
                        ...
                        // Vertical clamp only (not horizontal – horizontal skip is handled above)
                        if (drawCenterY < chartArea.top + verticalSafety) drawCenterY = chartArea.top + verticalSafety;
                        if (drawCenterY > chartArea.bottom - verticalSafety) drawCenterY = chartArea.bottom - verticalSafety;
                        ...
                        if (!isLine && height > 45) {
                            ...
                            ctx.fillText(nameToDraw, drawCenterX, drawCenterY - fontSize / 1.1);
                            ctx.font = `bold ${fontSize}px sans-serif`;
                            ctx.fillText(valStr, drawCenterX, drawCenterY + fontSize / 1.1);
                        } else {
                            ctx.fillText(valStr, drawCenterX, drawCenterY);
                        }
                    }
                    ...
                    if (window.showFinDiff && index > 0) {
                        ...
                        if (prevVal !== null && prevVal !== undefined) {
                            ...
                            const prevProps = prevMeta.getProps(['x', 'y', 'base'], true);
                            const prevX = prevProps.x;
                            const midX = (x + prevX) / 2;

                            // Skip diff pill if midpoint is outside the visible chart area
                            if (midX < chartArea.left || midX > chartArea.right) return;

                            // midY:
                            //   line → midpoint between rendered pixel Y of the two points (props.y)
                            //   bar  → midpoint between the two segment-top pixels (stack-aware)
                            let midY;
                            if (isLine) {
                                // props.y and prevProps.y are the actual rendered point pixels
                                midY = (props.y + prevProps.y) / 2;
                            } else {
                                // props.y = top of current segment, prevProps.y = top of previous segment
                                midY = (props.y + prevProps.y) / 2;
                            }
                            ...
                            // Vertical clamp only for diff pill
                            if (drawMidY - boxHeight / 2 < chartArea.top) drawMidY = chartArea.top + boxHeight / 2 + 2;
                            if (drawMidY + boxHeight / 2 > chartArea.bottom) drawMidY = chartArea.bottom - boxHeight / 2 - 2;

                            const drawBoxX = drawMidX - boxWidth / 2;
                            const drawBoxY = drawMidY - boxHeight / 2;
                            ...
                            ctx.beginPath();
                            const radius = 6;
                            if (ctx.roundRect) ctx.roundRect(drawBoxX, drawBoxY, boxWidth, boxHeight, radius);
                            else {
                                ctx.moveTo(drawBoxX + radius, drawBoxY);
                                ctx.lineTo(drawBoxX + boxWidth - radius, drawBoxY);
                                ctx.quadraticCurveTo(drawBoxX + boxWidth, drawBoxY, drawBoxX + boxWidth, drawBoxY + radius);
                                ctx.lineTo(drawBoxX + boxWidth, drawBoxY + boxHeight - radius);
                                ctx.quadraticCurveTo(drawBoxX + boxWidth, drawBoxY + boxHeight, drawBoxX + boxWidth - radius, drawBoxY + boxHeight);
                                ctx.lineTo(drawBoxX + radius, drawBoxY + boxHeight);
                                ctx.quadraticCurveTo(drawBoxX, drawBoxY + boxHeight, drawBoxX, drawBoxY + boxHeight - radius);
                                ctx.lineTo(drawBoxX, drawBoxY + radius);
                                ctx.quadraticCurveTo(drawBoxX, drawBoxY, drawBoxX + radius, drawBoxY);
                            }
                            ctx.closePath();
                            ctx.fill();
                            ...
                            ctx.textBaseline = 'middle';
                            ctx.textAlign = 'center';
                            ctx.fillText(diffStr, drawMidX, drawMidY + 1);
                        }
                    }
                    ctx.restore();
                });
    ...
    // Re-create chart for "rising up" animation when structure changes
    if (window.finChartInstance) window.finChartInstance.destroy();

    // Labels hidden until animation completes
    window._finLabelsReady = false;
    clearTimeout(window._finLabelTimer);
    // Default Chart.js bar animation is ~400 ms; add buffer
    window._finLabelTimer = setTimeout(() => {
        window._finLabelsReady = true;
        if (window.finChartInstance) window.finChartInstance.draw();
    }, 500);
        ...
        afterDatasetsDraw(chart) {
            ...
            chart.data.datasets.forEach((ds, i) => {
                ...
                if (isMatch) {
                    ...
                    if (window.finChartTypeLine || ds.type === 'line') {
                        // 1. Highlight Points for Line Chart
                        meta.data.forEach((pt) => {
                            ctx.save();
                            ctx.beginPath();
                            const radius = (pt.options && pt.options.radius) ? pt.options.radius : 3;
                            ...
                            // Moving Marquee effect on point border (optional, but glow is better for points)
                            ctx.shadowBlur = 0;
                        ...
                        // 2. Highlight Bars for Bar Chart
    ...
    const isStacked = (!window.finChartTypeLine) || (window.finChartTypeLine && window.finAreaChart);
    ...
    // Services-specific axes for independent scaling
    chartScales['y1'] = {
        stacked: false, beginAtZero: true, min: 0, position: 'right', display: false,
        grid: { drawOnChartArea: false },
        title: { display: true, text: 'ผู้ป่วยนอก (ครั้ง/คน)', font: { weight: 'bold', size: 10 } },
        ticks: { font: { size: 9 }, callback: v => formatAbbreviated(v) }
    };
    chartScales['y2'] = {
        stacked: false, beginAtZero: true, min: 0, position: 'right', display: false,
        grid: { drawOnChartArea: false },
        title: { display: true, text: 'ผู้ป่วยใน (วันนอน)', font: { weight: 'bold', size: 10 } },
        ticks: { font: { size: 9 }, callback: v => formatAbbreviated(v) }
    };
    chartScales['y3'] = {
        stacked: false, beginAtZero: true, min: 0, position: 'right', display: false,
        grid: { drawOnChartArea: false },
        title: { display: true, text: 'น้ำหนักสัมพัทธ์ (Sum AdjRW)', font: { weight: 'bold', size: 10 } },
        ticks: { font: { size: 9 }, callback: v => formatAbbreviated(v) }
    };
    chartScales['y4'] = {
        stacked: false, beginAtZero: true, min: 0, position: 'right', display: false,
        grid: { drawOnChartArea: false },
        title: { display: true, text: 'อัตราครองเตียง (%)', font: { weight: 'bold', size: 10 } },
        ticks: { font: { size: 9 } }
    };
    chartScales['y5'] = {
        stacked: false, beginAtZero: true, min: 0, position: 'right', display: false,
        grid: { drawOnChartArea: false },
        title: { display: true, text: 'Sum AdjRW / วันนอน', font: { weight: 'bold', size: 10 } },
...
function renderFinancialSidebar() {
    ...
    const sortedStructure = [...window.FIN_STRUCTURE].sort((a, b) => (a._displayOrder || 99) - (b._displayOrder || 99));
    sortedStructure.forEach(g => {
                ...
                <div style="margin-bottom: 24px; opacity: ${isStackChecked ? '1' : '0.6'}; transition: opacity 0.3s;">
                    ...
                    <div style="display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; padding-bottom:6px; border-bottom:2px solid ${g.color}; cursor: default;"
                        ...
                        <span style="font-weight:700; color:${g.color}; font-size:1rem; display:flex; align-items:center; gap:6px;">
                            ${g.label}
                            ${g.id === 'ServicesYearly' ? `<span onclick="event.stopPropagation(); window.reloadServiceStatistics()" title="โหลดข้อมูลใหม่" style="cursor:pointer; font-size:1.1rem; color:${g.color}; display:flex; align-items:center; justify-content:center; width:24px; height:24px; border-radius:50%; background:rgba(139, 92, 246, 0.1); margin-left:4px;">↻</span>` : ''}
                        </span>
        ...
        sortedSubgroups.forEach(sg => {
            ...
            if (isDetailed) {
                ...
                itemsToRender.forEach(item => {
                    ...
                    const isExpanded = window.finItemExpandedState && window.finItemExpandedState[item.key];
                            ...
                            <div style="margin-bottom: 4px;">
                                <div onclick="event.stopPropagation(); window.toggleFinSubItem('${item.key}', ${!isChecked})" 
                                    ...
                                    <div style="display:flex; align-items:center; gap:8px;">
                                        ...
                                        ${hasSubItems ? `
                                            <div onclick="event.stopPropagation(); window.toggleFinSubItemExpanded('${item.key}')" 
                                                 style="padding:2px; border-radius:4px; transition:background 0.2s; color:#94a3b8; transform: rotate(${isExpanded ? '180deg' : '0deg'});"
                                ...
                                ${hasSubItems && isExpanded ? `
                                    <div style="margin-left: 20px; padding-left: 10px; border-left: 1px dashed #cbd5e1; margin-top: 4px; display:grid; gap:4px;">
                                        ${item.subItems.map(si => {
                        const siChecked = window.finSubItemState ? (window.finSubItemState[si.key] === true) : (!si.hideInChart);
                        return `
                                                <div onclick="event.stopPropagation(); window.toggleFinSubItem('${si.key}', ${!siChecked})" 
                                                     class="fin-item-row"
                                                     onmouseover="window.highlightFinSegment('${si.key}')"
                                                     onmouseout="window.clearFinHighlight()"
                                                     style="cursor:pointer; display:flex; align-items:center; gap:8px; padding: 4px 8px; border-radius: 4px; transition: all 0.2s;
                                                            opacity: ${siChecked ? '1' : '0.6'}; 
                                                            background: ${siChecked ? 'rgba(255,255,255,0.7)' : 'transparent'};">
                                                    <div style="width: 8px; height: 8px; border-radius: 2px; background-color: ${si.color}; border: 1px solid rgba(0,0,0,0.1); flex-shrink:0;"></div>
                                                    <span style="font-size:0.75rem; color:#475569; flex:1; text-decoration:${siChecked ? 'none' : 'line-through'};">${si.label}</span>
                                                    ${si.hideInChart ? `<span style="font-size:0.65rem; color:${siChecked ? '#ef4444' : '#94a3b8'}; font-style:italic;">${siChecked ? '(แสดงรายการนี้เท่านั้น)' : '(ตารางเท่านั้น)'}</span>` : ''}
                                                </div>
                                            `;
                    }).join('')}
                                    </div>
...
window.toggleFinSubItem = function (itemKey, isChecked) {
    ...
    // Find if this is an "Exclusive" (hideInChart) item
    let isExclusiveItem = false;
    window.FIN_STRUCTURE.forEach(g => {
        g.subgroups.forEach(sg => {
            sg.items.forEach(it => {
                if (it.subItems) {
                    it.subItems.forEach(si => {
                        if (si.key === itemKey && si.hideInChart) isExclusiveItem = true;
                    });
                }
            });
        });
    ...
    if (isExclusiveItem && isChecked) {
        // TURN OFF ALL OTHER DATASETS
        window.FIN_STRUCTURE.forEach(g => {
            window.finStackState[g.id] = false;
        });
        Object.keys(window.finSubGroupVisibility || {}).forEach(k => {
            window.finSubGroupVisibility[k] = false;
        });
        Object.keys(window.finSubItemState || {}).forEach(k => {
            window.finSubItemState[k] = false;
        });

        // TURN ON PARENT PATH FOR THIS ITEM
        window.FIN_STRUCTURE.forEach(g => {
            g.subgroups.forEach(sg => {
                let hasItem = false;
                sg.items.forEach(it => {
                    if (it.subItems && it.subItems.some(si => si.key === itemKey)) hasItem = true;
                });
                if (hasItem) {
                    window.finStackState[g.id] = true;
                    window.finSubGroupVisibility[sg.id] = true;
                    window.finGroupState[sg.id] = 'detailed';
                }
            });
        });
    }
...
window.finItemExpandedState = {};
window.toggleFinSubItemExpanded = function (itemKey) {
    window.finItemExpandedState[itemKey] = !window.finItemExpandedState[itemKey];
    renderFinancialSidebar(); // Re-render to show children
};
...
window.highlightFinSegment = function (key) {
    ...
    // Apply Border Dash for Line Charts immediately for better feedback
    if (window.finChartTypeLine) {
        chart.data.datasets.forEach(ds => {
            if (!ds._isCmp) { // Only primary datasets
                const isMatch = (ds._customKey === key) || (ds._subgroupId === key) || (ds.stack === key);
                ds.borderWidth = isMatch ? 4 : 2;
                ds.borderDash = isMatch ? [6, 4] : [];
            }
        });
        chart.update('none');
    }
    ...
    if (key) {
        const loop = () => {
            ...
            if (window.finChartTypeLine) {
                chart.data.datasets.forEach(ds => {
                    if (!ds._isCmp && ds.borderDash && ds.borderDash.length > 0) {
                        ds.borderDashOffset = -(now / 20) % 20;
                    }
                });
                window.finChartInstance.update('none');
            } else {
...
window.highlightFinCompare = function (code) {
    if (window.activeCompareHover === code) return;
    window.activeCompareHover = code;

    // Only highlight if the line is currently being shown (was clicked and added to finCompareHospitals)
    if (!window.finCompareHospitals.includes(code)) return;
    ...
    chart.data.datasets.forEach(ds => {
        if (ds._isCmp) {
            const isMatch = ds._customKey && ds._customKey.endsWith('_cmp_' + code);
            ds.borderWidth = isMatch ? 4 : 2.5;
            ds.borderDash = isMatch ? [6, 4] : [];
        }
    });

    chart.update('none'); // Safe for 60fps, avoids path transition glitches

    if (finAnimLoop) cancelAnimationFrame(finAnimLoop);

    const loop = () => {
        if (!window.finChartInstance || (!window.activeFinHighlightKey && !window.activeCompareHover)) {
            if (finAnimLoop) cancelAnimationFrame(finAnimLoop);
            return;
        }
        const now = Date.now();
        window.finChartInstance.data.datasets.forEach(ds => {
            if (ds._isCmp && window.activeCompareHover && ds._customKey && ds._customKey.endsWith('_cmp_' + window.activeCompareHover)) {
                ds.borderDashOffset = -(now / 20) % 20;
            }
        });
        window.finChartInstance.update('none'); // Update style loop
        finAnimLoop = requestAnimationFrame(loop);
    };
    finAnimLoop = requestAnimationFrame(loop);
};

window.highlightFinPrimary = function () {
    if (window.activePrimaryHover) return;
    window.activePrimaryHover = true;
    ...
    chart.data.datasets.forEach(ds => {
        if (!ds._isCmp) {
            ds.borderWidth = window.finChartTypeLine ? 4 : 2;
            ds.borderDash = [6, 4];
        }
    });
    ...
    const loop = () => {
        if (!window.finChartInstance || (!window.activeFinHighlightKey && !window.activeCompareHover && !window.activePrimaryHover)) {
            if (finAnimLoop) cancelAnimationFrame(finAnimLoop);
            return;
        }
        const now = Date.now();
        window.finChartInstance.data.datasets.forEach(ds => {
            if (ds._isCmp && window.activeCompareHover && ds._customKey && ds._customKey.endsWith('_cmp_' + window.activeCompareHover)) {
                ds.borderDashOffset = -(now / 20) % 20;
            } else if (!ds._isCmp && window.activePrimaryHover) {
                ds.borderDashOffset = -(now / 20) % 20;
            }
        });
        window.finChartInstance.update('none');
...
window.clearFinCompareHighlight = function () {
    window.activeCompareHover = null;
    window.activePrimaryHover = false;
    ...
    if (chart) {
        chart.data.datasets.forEach(ds => {
            if (ds._isCmp) {
                ...
                ds.borderWidth = window.finChartTypeLine ? 2 : 1;
                ds.borderDash = [];
            }
        });
        chart.update('none');
    }
    window.clearFinHighlight();
};
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('mouseup', (e) => {
        ...
        if (modalMouseDownTarget === upTarget && upTarget.classList.contains('modal')) {
            if (upTarget.id === 'finChartModal') closeFinancialChartModal();
            if (upTarget.id === 'reportModal') closeReportModal();
            if (upTarget.id === 'chartModal') closeChartModal();
        }
