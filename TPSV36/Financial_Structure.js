        ...
        afterDatasetsDraw(chart, args, options) {
            ...
            chart.data.datasets.forEach((dataset, i) => {
                ...
                meta.data.forEach((element, index) => {
                    ...
                    const height = isLine ? 20 : model.height;
                    const width = isLine ? 40 : model.width;
                    const x = model.x;
                    const y = model.y;
                    const base = model.base || y;
                    
                    // STAGGER LOGIC: If multiple line datasets, offset them vertically to avoid overlapping
                    // We use dataset index 'i' to stagger.
                    const staggerOffset = isLine ? (i * 18) : 0;
                    const centerY = isLine ? y - 12 - staggerOffset : (y + base) / 2;
                    ...
                    if (window.showFinValues && height > 15) {
                        ...
                        // Boundary Check for Value Label
                        let drawCenterX = x;
                        let drawCenterY = centerY;
                        const verticalSafety = 10;
                        const horizontalSafety = 30; // Safety for text width
                        ...
                        if (drawCenterX < chartArea.left + horizontalSafety) drawCenterX = chartArea.left + horizontalSafety;
                        if (drawCenterX > chartArea.right - horizontalSafety) drawCenterX = chartArea.right - horizontalSafety;
                    ...
                    if (window.showFinDiff && index > 0) {
                        ...
                        if (prevVal !== null && prevVal !== undefined) {
                            ...
                            // Staggered midY to match staggered points
                            ...
                            // Horizontal Boundary Check for Diff Pill
                            if (drawMidX - boxWidth / 2 < chartArea.left) drawMidX = chartArea.left + boxWidth / 2 + 2;
                            if (drawMidX + boxWidth / 2 > chartArea.right) drawMidX = chartArea.right - boxWidth / 2 - 2;

                            // Vertical Boundary Check for Diff Pill
