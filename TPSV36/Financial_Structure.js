...
async function selectHospital(code, silent = false) {
    ...
    try {
        ...
        yearsToFetch.forEach(y => {
            requestPromises.push(jsonpRequest('getMgmtData', `code=${code}&year=${y}`).catch(e => ({ headers: [], data: {} })));
        ...
        let combinedTpsData = {};
        let combinedMgmtData = {};
        let combinedTpsHeaders = {};
        let combinedMgmtHeaders = {};
        let primaryTpsHeaders = [];

        results.forEach((res, idx) => {
            const yearIdx = Math.floor(idx / 2);
            const isMgmt = (idx % 2 === 0);
            const y = yearsToFetch[yearIdx];

            if (res.data) {
                // Filter data to only include rows for the requested year if 'ปี' column exists
                ...
                if (isMgmt) {
                    combinedMgmtData = { ...combinedMgmtData, ...filteredData };
                } else {
            ...
            if (res.headers && res.headers.length > 0) {
                if (isMgmt) {
                    combinedMgmtHeaders[y] = res.headers;
                } else {
                    combinedTpsHeaders[y] = res.headers;
                    if (yearIdx === 0) primaryTpsHeaders = res.headers;
                }
            }
        });

        const result = { headers: primaryTpsHeaders, data: combinedTpsData };
        ...
        if (item) {
            ...
            item.mgmtData = { ...item.mgmtData, ...combinedMgmtData };
            Object.assign(item.mgmtHeadersMap, combinedMgmtHeaders);
                    ...
                    <div class="modal-content" style="max-width:1400px; width:95%; height:95vh; display: flex; flex-direction: column;">
                        ...
                        <div id="finChartControls" class="modal-controls">
                             ...
                             <div class="control-group" style="display:flex; align-items:center; gap:8px;" onclick="document.getElementById('finNonCumulativeToggle').click()">
                                ...
                                <span class="control-label">แสดงค่ารายไตรมาส (ไม่สะสม)</span>
                             </div>
                        </div>
...
function updateFinancialChart() {
    ...
    if (!window.globalFinSubItemState) {
        ...
        window.FIN_STRUCTURE.forEach(g => g.subgroups.forEach(sg => sg.items.forEach(item => {
            // Default OFF for all Services items as requested
            if (g.id === 'Services') {
    ...
    if (!window.globalFinSubGroupVisibility) {
        ...
        window.FIN_STRUCTURE.forEach(g => g.subgroups.forEach(sg => {
            if (g.id === 'Services') {
                // Services group: all items hidden by default as requested
        ...
        sortedSubgroups.forEach(subgroup => {
            ...
            if (mode === 'summary') {
                ...
                datasets.push({
                    ...
                    yAxisID: isServicesGroup ? (subgroup.id === 'BedOccupancy_Group' ? 'y2' : 'y1') : 'y'
                });
            } else {
                ...
                items.forEach(item => {
                    ...
                    datasets.push({
                        ...
                        _customKey: item.key,
                        _subgroupId: subgroup.id,
                        yAxisID: isServicesGroup ? (subgroup.id === 'BedOccupancy_Group' || item.key === 'BedOccupancy' ? 'y2' : 'y1') : 'y'
                    });
                });
            }
        });
        ...
        options: {
            ...
            scales: {
                ...
                y2: {
                    ...
                    display: 'auto',
                    grid: { drawOnChartArea: false },
                    title: { display: true, text: 'อัตราครองเตียง (%)', font: { weight: 'bold' } }
                }
            },
            plugins: {
                zoom: {
