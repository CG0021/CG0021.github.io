...
const Auth = {
    ...
    getQuotaRemain() {
        return parseInt(localStorage.getItem("userRemain") || "0");
    },
    getQuotaLimit() {
        return parseInt(localStorage.getItem("userLimit") || "0");
    },
    setSession(username, limit, remain, name) {
        localStorage.setItem("currentUser", username);
        localStorage.setItem("userLimit", limit);
        localStorage.setItem("userRemain", remain);
        if (name !== undefined) {
    ...
    clearSession() {
        ...
        localStorage.removeItem("userRemain");
...
document.addEventListener("DOMContentLoaded", () => {
    Auth.clearSession(); // เคลียร์เซสชันทุกครั้งที่โหลดหน้าเว็บ/รีเฟรช เพื่อบังคับล็อกอินใหม่
    setupAuthUI();
    checkLoginState();
});
...
// Update User Profile & Quota info in Header
function updateHeaderProfile() {
    ...
    profileContainer.innerHTML = `
        <div class="flex items-center gap-2 bg-slate-50 px-2.5 py-1 rounded-xl border border-slate-200/60 shadow-sm">
            ...
            <button id="btnOpenChangePassword" class="text-slate-500 hover:text-sky-600 transition-colors focus:outline-none border border-slate-200 hover:bg-slate-100 rounded-lg shadow-sm flex items-center justify-center w-7 h-7" title="เปลี่ยนรหัสผ่าน">
                <span>🔑</span>
            </button>
    ...
    if (loginForm) {
        loginForm.addEventListener("submit", async (e) => {
            ...
            try {
                ...
                if (result.success) {
                    Auth.setSession(result.username, result.limit, result.remain, result.name);
                    usernameInput.value = "";
                    passwordInput.value = "";
                    checkLoginState(true);
                } else {
