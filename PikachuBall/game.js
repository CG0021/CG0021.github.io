...
class GameEngine {
    ...
    update(inputs, excludeSlots = []) {
        // Apply input movement for each player (always allowed unless excluded)
        for (let id = 1; id <= 4; id++) {
            if (excludeSlots.includes(id)) continue;
            const input = inputs[id] || {};
            this.updatePlayer(id, input);
        }
        ...
        ball.vy += GRAVITY * 0.32; // Ball floats nicely! (Even softer)
        ball.x += ball.vx;
        ball.y += ball.vy;
        ...
        if (ball.x - BALL_RADIUS < 0) {
            ball.x = BALL_RADIUS;
            ball.vx = -ball.vx * 0.6; // Soft bounce
        ...
        if (ball.y - BALL_RADIUS < 0) {
            ball.y = BALL_RADIUS;
            ball.vy = -ball.vy * 0.6; // Soft bounce
        ...
        for (let id = 1; id <= 4; id++) {
            ...
            if (dist < minDist) {
                ...
                // Bounce dynamics (Slightly slower for better playability)
                let force = 4.2;
                
                // If spiking (down direction)
                if (input.down) {
                    force = 7.5;
                    ball.vx = nx * force * 1.1;
                    ball.vy = Math.abs(ny) * force * 0.7 + 2; // reactable spike downward
